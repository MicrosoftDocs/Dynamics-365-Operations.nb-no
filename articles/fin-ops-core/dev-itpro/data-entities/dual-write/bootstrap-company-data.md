...
---
# <a name="bootstrap-with-company-data-faq"></a><span data-ttu-id="e5555-103">Vanlige spørsmål om oppstart med firmadata</span><span class="sxs-lookup"><span data-stu-id="e5555-103">Bootstrap with company data FAQ</span></span>
 
[!include [banner](../../includes/banner.md)]


## <a name="why-do-i-need-bootstrapping"></a><span data-ttu-id="e5555-104">Hvorfor trenger jeg å bruke oppstart?</span><span class="sxs-lookup"><span data-stu-id="e5555-104">Why do I need bootstrapping?</span></span> 
<span data-ttu-id="e5555-105">Du kan ha en eksisterende Common Data Service eller en annen Dynamics 365-appforekomst med forretningsdata, og du vil aktivere dobbel skriving-forbindelse mot den.</span><span class="sxs-lookup"><span data-stu-id="e5555-105">You might have an existing Common Data Service or other Dynamics 365 app instance with business data, and you want to enable dual-write connection against it.</span></span> <span data-ttu-id="e5555-106">I dette tilfellet må du starte opp Common Data Service eller andre Dynamics 365-appdata med firmainformasjon før aktivering av dobbel skriving-tilkobling.</span><span class="sxs-lookup"><span data-stu-id="e5555-106">In this case, you need to bootstrap Common Data Service or other Dynamics 365 app data with company information before enabling dual-write connection.</span></span>  
 
## <a name="when-should-i-use-bootstrapping"></a><span data-ttu-id="e5555-107">Når bør jeg bruke oppstart?</span><span class="sxs-lookup"><span data-stu-id="e5555-107">When should I use bootstrapping?</span></span> 
<span data-ttu-id="e5555-108">Du bør bruke oppstart før du aktiverer dobbel skriving-enhetstilordninger (under trinn 5).</span><span class="sxs-lookup"><span data-stu-id="e5555-108">You should use bootstrapping before enabling dual-write entity maps (during step #5).</span></span>  
1. <span data-ttu-id="e5555-109">Hvis du vil konfigurere dobbel skriving-forbindelsen mellom forekomster av Finance and Operations-appen og Common Data Service eller den andre Dynamics 365-appen, logger du på Finance and Operations-appen som administrator.</span><span class="sxs-lookup"><span data-stu-id="e5555-109">To setup the dual-write connection between instances of your Finance and Operations app and the Common Data Service or other Dynamics 365 app, log in to the Finance and Operations app as an administrator.</span></span> 
2. <span data-ttu-id="e5555-110">Gå til **Databehandling**-modulen, og klikk **Dobbel skriving**-knappen.</span><span class="sxs-lookup"><span data-stu-id="e5555-110">Go to the **Data Management** module and click the **Dual-Write** button.</span></span> <span data-ttu-id="e5555-111">Dette starter **dataintegratoren**.</span><span class="sxs-lookup"><span data-stu-id="e5555-111">This launches the **Data Integrator**.</span></span> 
3. <span data-ttu-id="e5555-112">Opprett dobbel skriving-tilkoblingen for ett eller flere firmaer.</span><span class="sxs-lookup"><span data-stu-id="e5555-112">Create the dual-write connection for one or more companies.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="e5555-113">![Opprett dobbel skriving-tilkobling](media/dual-write-boot-1.png)</span><span class="sxs-lookup"><span data-stu-id="e5555-113">![Create dual-write connection](media/dual-write-boot-1.png)</span></span>
4. <span data-ttu-id="e5555-114">Aktiver enhetstilordningen for **Cdm_companies**.</span><span class="sxs-lookup"><span data-stu-id="e5555-114">Enable the **Cdm_companies** entity map.</span></span> <span data-ttu-id="e5555-115">Dette synkroniserer firmaer fra Finance and Operations-appen til Common Data Service.</span><span class="sxs-lookup"><span data-stu-id="e5555-115">This synchronizes companies from the Finance and Operations app to Common Data Service.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="e5555-116">![Aktivere enhetstilordningen](media/dual-write-boot-2.png)</span><span class="sxs-lookup"><span data-stu-id="e5555-116">![Enable the entity map](media/dual-write-boot-2.png)</span></span>
5. <span data-ttu-id="e5555-117">Kjør eksemplet på oppstartskode på Common Data Service eller den andre Dynamics 365-appforekomsten.</span><span class="sxs-lookup"><span data-stu-id="e5555-117">Run the sample bootstrapping code on the Common Data Service or other Dynamics 365 app instance.</span></span>  
6. <span data-ttu-id="e5555-118">Når oppstartfunksjonen er utført og systemet er klart for Live-synkronisering, aktiverer du enhetstilordningene.</span><span class="sxs-lookup"><span data-stu-id="e5555-118">When the bootstrapping is done and the system is ready for the live sync, enable the entity maps.</span></span>  

    <span data-ttu-id="e5555-119">Aktivering av enhetstilordningen utløser den opprinnelige datasynkroniseringen for enhetstilordninger som er aktivert.</span><span class="sxs-lookup"><span data-stu-id="e5555-119">Enabling the entity maps triggers the initial data sync for the enabled entity maps.</span></span> <span data-ttu-id="e5555-120">Dataene som tilsvarer firmaene som er valgt på dobbel skriving-tilkoblingen, er synkronisert mellom Finance and Operations-appen og Common Data Service.</span><span class="sxs-lookup"><span data-stu-id="e5555-120">The data corresponding to the companies chosen on the dual-write connection is synchronized between the Finance and Operations app and Common Data Service.</span></span> 
 
## <a name="how-to-i-use-the-code-sample"></a><span data-ttu-id="e5555-121">Hvordan bruker jeg kodeeksemplet?</span><span class="sxs-lookup"><span data-stu-id="e5555-121">How to I use the code sample?</span></span>
<span data-ttu-id="e5555-122">Eksempelkoden er et C#-program du kan laste inn i Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="e5555-122">The sample code is a C# application that you can load in Visual Studio.</span></span> <span data-ttu-id="e5555-123">Den tar NuGet-pakkeavhengigheter på Common Data Service SDK, som du kan oppdatere ved hjelp av standardverktøy for Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="e5555-123">It takes NuGet package dependencies on the Common Data Service SDK, that you can refresh through standard Visual Studio tooling.</span></span> 

<span data-ttu-id="e5555-124">Når du har pakket ut og åpnet løsningen i Visual Studio og gjenopprettet NuGet-pakkene, kan du søke etter **GJØREMÅL** i koden.</span><span class="sxs-lookup"><span data-stu-id="e5555-124">After unzipping and opening the solution in Visual Studio and restoring the NuGet packages, search for **TODO** in the code.</span></span> <span data-ttu-id="e5555-125">Hver avgjørelse du må ta om hvordan du vil starte opp firmainformasjon, er angitt av et **GJØREMÅL**, med eksempelkode for en kanonisk implementering.</span><span class="sxs-lookup"><span data-stu-id="e5555-125">Each decision you need to make about how you want to bootstrap company information is noted by a **TODO**, with sample code for a canonical implementation.</span></span> 

<span data-ttu-id="e5555-126">Eksempelkoden viser bare én av mange måter du kan kategorisere enhetsposter etter firma på.</span><span class="sxs-lookup"><span data-stu-id="e5555-126">The sample code shows only one of many ways you might categorize entity records by company.</span></span> <span data-ttu-id="e5555-127">Ved å endre logikken i **GJØREMÅL**-delene kan du opprette en egendefinert kategorisering.</span><span class="sxs-lookup"><span data-stu-id="e5555-127">By changing the logic in the **TODO** sections, you can create your custom categorization.</span></span> 
 
## <a name="what-should-i-expect"></a><span data-ttu-id="e5555-128">Hva bør jeg forvente?</span><span class="sxs-lookup"><span data-stu-id="e5555-128">What should I expect?</span></span>
<span data-ttu-id="e5555-129">Som standard kan du bruke eksempelprogrammet til å angi en ordliste med kodetilordninger for forretningsenhet til firma.</span><span class="sxs-lookup"><span data-stu-id="e5555-129">By default, the sample application lets you provide a dictionary of business unit-to-company code mappings.</span></span> <span data-ttu-id="e5555-130">En hvilken som helst enhet som starter med et **OwningBusinessUnit**-felt, angis automatisk til å bruke det angitte firmaet.</span><span class="sxs-lookup"><span data-stu-id="e5555-130">Any entity you bootstrap with an **OwningBusinessUnit** field is automatically set to use the specified company.</span></span> <span data-ttu-id="e5555-131">Alle enheter uten et **OwningBusinessUnit**-felt, for eksempel produkt, vil angi firmaet basert på tilordningen med en tom forretningsenhetsverdi.</span><span class="sxs-lookup"><span data-stu-id="e5555-131">Any entity without an **OwningBusinessUnit** field, such as product, will set the company based on the mapping with an empty business unit value.</span></span>

<span data-ttu-id="e5555-132">Konsollprogrammet forventer én parameter, enten **–simulate** eller **–apply**.</span><span class="sxs-lookup"><span data-stu-id="e5555-132">The console application expects one parameter, either **–simulate** or **–apply**.</span></span> <span data-ttu-id="e5555-133">Hvis du bruker kommandolinjeparameteren **–simulate**, oppdateres ingen data.</span><span class="sxs-lookup"><span data-stu-id="e5555-133">If you use the **–simulate** command line parameter, then no data is updated.</span></span> <span data-ttu-id="e5555-134">Bare **simulation_<entityname>.csv**-filer genereres i samme mappe som verktøyet, en for hver enhet som ville ha blitt oppdatert.</span><span class="sxs-lookup"><span data-stu-id="e5555-134">Only **simulation_<entityname>.csv** files are generated in the same directory as the tool, one for each entity that would have been updated.</span></span> <span data-ttu-id="e5555-135">Du kan iterativt se gjennom disse filene når du arbeider for å sikre at koden oppdaterer firmaverdier som forventet.</span><span class="sxs-lookup"><span data-stu-id="e5555-135">You can iteratively review these files while working to ensure the code updates company values as expected.</span></span> 

<span data-ttu-id="e5555-136">Når du er ferdig med de simulerte oppdateringene, bruker du parameteren **–apply**.</span><span class="sxs-lookup"><span data-stu-id="e5555-136">When you finish with the simulated updates, then use the **–apply** parameter.</span></span> <span data-ttu-id="e5555-137">Dette oppdaterer alle poster som har feil firmaverdi, i grupper med 1000 poster om gangen (som standard).</span><span class="sxs-lookup"><span data-stu-id="e5555-137">This updates all records that currently have an incorrect company value, in batches of 1000 records at a time (by default).</span></span> <span data-ttu-id="e5555-138">Koden er idempotent som oppgitt, noe som betyr at du kan kjøre den på nytt, og bare de feilaktig tilordnede selskapene blir oppdatert.</span><span class="sxs-lookup"><span data-stu-id="e5555-138">The code is idempotent as provided, meaning you can re-run it and only the incorrectly assigned companies will be updated.</span></span> <span data-ttu-id="e5555-139">Når det kjøres med **–apply**, vil koden skrive ut CSV-filer for endringene som er gjort, som kalles **applied_<entityname>.csv.**</span><span class="sxs-lookup"><span data-stu-id="e5555-139">When running with **–apply**, the code outputs CSV files of the changes made, which are named **applied_<entityname>.csv**.</span></span> 
