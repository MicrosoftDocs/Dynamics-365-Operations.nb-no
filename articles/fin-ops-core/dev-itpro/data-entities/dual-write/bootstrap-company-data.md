...
description: Hvordan du starter opp Dataverse eller andre Dynamics 365-appdata med firmainformasjon før aktivering av dobbel skriving-tilkobling.
...
---
# <a name="bootstrap-with-company-data-faq"></a><span data-ttu-id="fde8e-103">Vanlige spørsmål om oppstart med firmadata</span><span class="sxs-lookup"><span data-stu-id="fde8e-103">Bootstrap with company data FAQ</span></span>
 
[!include [banner](../../includes/banner.md)]

[!include [rename-banner](~/includes/cc-data-platform-banner.md)]

## <a name="why-do-i-need-bootstrapping"></a><span data-ttu-id="fde8e-104">Hvorfor trenger jeg å bruke oppstart?</span><span class="sxs-lookup"><span data-stu-id="fde8e-104">Why do I need bootstrapping?</span></span> 
<span data-ttu-id="fde8e-105">Du kan ha en eksisterende Dataverse eller en annen Dynamics 365-appforekomst med forretningsdata, og du vil aktivere dobbel skriving-forbindelse mot den.</span><span class="sxs-lookup"><span data-stu-id="fde8e-105">You might have an existing Dataverse or other Dynamics 365 app instance with business data, and you want to enable dual-write connection against it.</span></span> <span data-ttu-id="fde8e-106">I dette tilfellet må du starte opp Dataverse eller andre Dynamics 365-appdata med firmainformasjon før aktivering av dobbel skriving-tilkobling.</span><span class="sxs-lookup"><span data-stu-id="fde8e-106">In this case, you need to bootstrap Dataverse or other Dynamics 365 app data with company information before enabling dual-write connection.</span></span>  
 
## <a name="when-should-i-use-bootstrapping"></a><span data-ttu-id="fde8e-107">Når bør jeg bruke oppstart?</span><span class="sxs-lookup"><span data-stu-id="fde8e-107">When should I use bootstrapping?</span></span> 
<span data-ttu-id="fde8e-108">Du bør bruke oppstart før du aktiverer dobbel skriving-tabelltilordninger (under trinn 5).</span><span class="sxs-lookup"><span data-stu-id="fde8e-108">You should use bootstrapping before enabling dual-write table maps (during step #5).</span></span>  
1. <span data-ttu-id="fde8e-109">Hvis du vil konfigurere dobbel skriving-forbindelsen mellom forekomster av Finance and Operations-appen og Dataverse eller den andre Dynamics 365-appen, logger du på Finance and Operations-appen som administrator.</span><span class="sxs-lookup"><span data-stu-id="fde8e-109">To setup the dual-write connection between instances of your Finance and Operations app and the Dataverse or other Dynamics 365 app, log in to the Finance and Operations app as an administrator.</span></span> 
2. <span data-ttu-id="fde8e-110">Gå til **Databehandling**-modulen, og klikk **Dobbel skriving**-knappen.</span><span class="sxs-lookup"><span data-stu-id="fde8e-110">Go to the **Data Management** module and click the **Dual-Write** button.</span></span> <span data-ttu-id="fde8e-111">Dette starter **dataintegratoren**.</span><span class="sxs-lookup"><span data-stu-id="fde8e-111">This launches the **Data Integrator**.</span></span> 
3. <span data-ttu-id="fde8e-112">Opprett dobbel skriving-tilkoblingen for ett eller flere firmaer.</span><span class="sxs-lookup"><span data-stu-id="fde8e-112">Create the dual-write connection for one or more companies.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="fde8e-113">![Opprett dobbel skriving-tilkobling](media/dual-write-boot-1.png)</span><span class="sxs-lookup"><span data-stu-id="fde8e-113">![Create dual-write connection](media/dual-write-boot-1.png)</span></span>
4. <span data-ttu-id="fde8e-114">Aktiver tabelltilordningen for **Cdm_companies**.</span><span class="sxs-lookup"><span data-stu-id="fde8e-114">Enable the **Cdm_companies** table map.</span></span> <span data-ttu-id="fde8e-115">Dette synkroniserer firmaer fra Finance and Operations-appen til Dataverse.</span><span class="sxs-lookup"><span data-stu-id="fde8e-115">This synchronizes companies from the Finance and Operations app to Dataverse.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="fde8e-116">![Aktivere tabelltilordningen](media/dual-write-boot-2.png)</span><span class="sxs-lookup"><span data-stu-id="fde8e-116">![Enable the table map](media/dual-write-boot-2.png)</span></span>
5. <span data-ttu-id="fde8e-117">Kjør eksemplet på oppstartskode på Dataverse eller den andre Dynamics 365-appforekomsten.</span><span class="sxs-lookup"><span data-stu-id="fde8e-117">Run the sample bootstrapping code on the Dataverse or other Dynamics 365 app instance.</span></span>  
6. <span data-ttu-id="fde8e-118">Når oppstartfunksjonen er utført og systemet er klart for Live-synkronisering, aktiverer du tabelltilordningene.</span><span class="sxs-lookup"><span data-stu-id="fde8e-118">When the bootstrapping is done and the system is ready for the live sync, enable the table maps.</span></span>  

    <span data-ttu-id="fde8e-119">Aktivering av tabelltilordningen utløser den opprinnelige datasynkroniseringen for tabelltilordninger som er aktivert.</span><span class="sxs-lookup"><span data-stu-id="fde8e-119">Enabling the table maps triggers the initial data sync for the enabled table maps.</span></span> <span data-ttu-id="fde8e-120">Dataene som tilsvarer firmaene som er valgt på dobbel skriving-tilkoblingen, er synkronisert mellom Finance and Operations-appen og Dataverse.</span><span class="sxs-lookup"><span data-stu-id="fde8e-120">The data corresponding to the companies chosen on the dual-write connection is synchronized between the Finance and Operations app and Dataverse.</span></span> 
 
## <a name="how-to-i-use-the-code-sample"></a><span data-ttu-id="fde8e-121">Hvordan bruker jeg kodeeksemplet?</span><span class="sxs-lookup"><span data-stu-id="fde8e-121">How to I use the code sample?</span></span>
<span data-ttu-id="fde8e-122">Eksempelkoden er et C#-program du kan laste inn i Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="fde8e-122">The sample code is a C# application that you can load in Visual Studio.</span></span> <span data-ttu-id="fde8e-123">Den tar NuGet-pakkeavhengigheter på Dataverse SDK, som du kan oppdatere ved hjelp av standardverktøy for Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="fde8e-123">It takes NuGet package dependencies on the Dataverse SDK, that you can refresh through standard Visual Studio tooling.</span></span> 

<span data-ttu-id="fde8e-124">Når du har pakket ut og åpnet løsningen i Visual Studio og gjenopprettet NuGet-pakkene, kan du søke etter **GJØREMÅL** i koden.</span><span class="sxs-lookup"><span data-stu-id="fde8e-124">After unzipping and opening the solution in Visual Studio and restoring the NuGet packages, search for **TODO** in the code.</span></span> <span data-ttu-id="fde8e-125">Hver avgjørelse du må ta om hvordan du vil starte opp firmainformasjon, er angitt av et **GJØREMÅL**, med eksempelkode for en kanonisk implementering.</span><span class="sxs-lookup"><span data-stu-id="fde8e-125">Each decision you need to make about how you want to bootstrap company information is noted by a **TODO**, with sample code for a canonical implementation.</span></span> 

<span data-ttu-id="fde8e-126">Eksempelkoden viser bare én av mange måter du kan kategorisere enhetsrader etter firma på.</span><span class="sxs-lookup"><span data-stu-id="fde8e-126">The sample code shows only one of many ways you might categorize entity rows by company.</span></span> <span data-ttu-id="fde8e-127">Ved å endre logikken i **GJØREMÅL**-delene kan du opprette en egendefinert kategorisering.</span><span class="sxs-lookup"><span data-stu-id="fde8e-127">By changing the logic in the **TODO** sections, you can create your custom categorization.</span></span> 
 
## <a name="what-should-i-expect"></a><span data-ttu-id="fde8e-128">Hva bør jeg forvente?</span><span class="sxs-lookup"><span data-stu-id="fde8e-128">What should I expect?</span></span>
<span data-ttu-id="fde8e-129">Som standard kan du bruke eksempelprogrammet til å angi en ordliste med kodetilordninger for forretningsenhet til firma.</span><span class="sxs-lookup"><span data-stu-id="fde8e-129">By default, the sample application lets you provide a dictionary of business unit-to-company code mappings.</span></span> <span data-ttu-id="fde8e-130">En hvilken som helst enhet som starter med et **OwningBusinessUnit**-felt, angis automatisk til å bruke det angitte firmaet.</span><span class="sxs-lookup"><span data-stu-id="fde8e-130">Any entity you bootstrap with an **OwningBusinessUnit** field is automatically set to use the specified company.</span></span> <span data-ttu-id="fde8e-131">Alle enheter uten et **OwningBusinessUnit**-felt, for eksempel produkt, vil angi firmaet basert på tilordningen med en tom forretningsenhetsverdi.</span><span class="sxs-lookup"><span data-stu-id="fde8e-131">Any entity without an **OwningBusinessUnit** field, such as product, will set the company based on the mapping with an empty business unit value.</span></span>

<span data-ttu-id="fde8e-132">Konsollprogrammet forventer én parameter, enten **–simulate** eller **–apply**.</span><span class="sxs-lookup"><span data-stu-id="fde8e-132">The console application expects one parameter, either **–simulate** or **–apply**.</span></span> <span data-ttu-id="fde8e-133">Hvis du bruker kommandolinjeparameteren **–simulate**, oppdateres ingen data.</span><span class="sxs-lookup"><span data-stu-id="fde8e-133">If you use the **–simulate** command line parameter, then no data is updated.</span></span> <span data-ttu-id="fde8e-134">Bare **simulation_<entityname>.csv**-filer genereres i samme mappe som verktøyet, en for hver enhet som ville ha blitt oppdatert.</span><span class="sxs-lookup"><span data-stu-id="fde8e-134">Only **simulation_<entityname>.csv** files are generated in the same directory as the tool, one for each entity that would have been updated.</span></span> <span data-ttu-id="fde8e-135">Du kan iterativt se gjennom disse filene når du arbeider for å sikre at koden oppdaterer firmaverdier som forventet.</span><span class="sxs-lookup"><span data-stu-id="fde8e-135">You can iteratively review these files while working to ensure the code updates company values as expected.</span></span> 

<span data-ttu-id="fde8e-136">Når du er ferdig med de simulerte oppdateringene, bruker du parameteren **–apply**.</span><span class="sxs-lookup"><span data-stu-id="fde8e-136">When you finish with the simulated updates, then use the **–apply** parameter.</span></span> <span data-ttu-id="fde8e-137">Dette oppdaterer alle rader som har feil firmaverdi, i grupper med 1000 rader om gangen (som standard).</span><span class="sxs-lookup"><span data-stu-id="fde8e-137">This updates all rows that currently have an incorrect company value, in batches of 1000 rows at a time (by default).</span></span> <span data-ttu-id="fde8e-138">Koden er idempotent som oppgitt, noe som betyr at du kan kjøre den på nytt, og bare de feilaktig tilordnede selskapene blir oppdatert.</span><span class="sxs-lookup"><span data-stu-id="fde8e-138">The code is idempotent as provided, meaning you can re-run it and only the incorrectly assigned companies will be updated.</span></span> <span data-ttu-id="fde8e-139">Når det kjøres med **–apply**, vil koden skrive ut CSV-filer for endringene som er gjort, som kalles **applied_<entityname>.csv.**</span><span class="sxs-lookup"><span data-stu-id="fde8e-139">When running with **–apply**, the code outputs CSV files of the changes made, which are named **applied_<entityname>.csv**.</span></span> 
