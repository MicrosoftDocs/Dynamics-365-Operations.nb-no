...
- <span data-ttu-id="9b327-107">Forstå installasjonen og den nødvendige konfigurasjonen som kreves for de ulike programmene som støtter RSAT (Regression Suite Automation Tool).</span><span class="sxs-lookup"><span data-stu-id="9b327-107">Understand the installation and prerequisite setup that are required for the various applications that support Regression suite automation tool (RSAT).</span></span>
...
## <a name="configure-azure-devops"></a><span data-ttu-id="9b327-123">Konfigurer Azure DevOps</span><span class="sxs-lookup"><span data-stu-id="9b327-123">Configure Azure DevOps</span></span>
...
<span data-ttu-id="9b327-173">For LCS-prosjektet kan du bruke en eksisterende kundeimplementering eller et eksisterende partnerprosjekt.</span><span class="sxs-lookup"><span data-stu-id="9b327-173">For your LCS project, you can use an existing customer implementation or partner project.</span></span>
...
8. <span data-ttu-id="9b327-270">Hvis du vil lagre oppgaveopptaket lokalt og deretter laste det opp til BPM via LCS, følger du denne fremgangsmåten:</span><span class="sxs-lookup"><span data-stu-id="9b327-270">If you want to save the task recording locally and then upload it to BPM through LCS, follow these steps:</span></span>
    ...
    3. <span data-ttu-id="9b327-275">Gå til BPM-biblioteket **RSAT**, og velg forretningsprosessen du vil lagre oppgaveopptaket mot.</span><span class="sxs-lookup"><span data-stu-id="9b327-275">Go to the **RSAT** BPM library, and select the business process to save the task recording against.</span></span>
...
3. <span data-ttu-id="9b327-326">Høyreklikk på **Utforsk** for å åpne mappen **\<Drive\>: \\AosService\\WebRoot**.</span><span class="sxs-lookup"><span data-stu-id="9b327-326">Right-click **Explore** to open the **\<Drive\>: \\AosService\\WebRoot** folder.</span></span> <span data-ttu-id="9b327-327">Finn filen **wif.config**.</span><span class="sxs-lookup"><span data-stu-id="9b327-327">Find the **wif.config** file.</span></span>
...
## <a name="get-started-with-rsat"></a><span data-ttu-id="9b327-352">Komme i gang med RSAT</span><span class="sxs-lookup"><span data-stu-id="9b327-352">Get started with RSAT</span></span>
...
1. <span data-ttu-id="9b327-419">Velg **Last inn** for å laste inn **RSAT-testplan** fra Azure DevOps-prosjektet.</span><span class="sxs-lookup"><span data-stu-id="9b327-419">Select **Load** to load the **RSAT Test Plan** test plan from the Azure DevOps project.</span></span>
...
5. <span data-ttu-id="9b327-486">Velg **Last opp** for å lagre Excel-parameterfilen i Azure DevOps.</span><span class="sxs-lookup"><span data-stu-id="9b327-486">Select **Upload** to save the Excel parameter file to Azure DevOps.</span></span>
    ...
    > [!NOTE]
    > <span data-ttu-id="9b327-488">Hvis du vil kjøre testtilfeller i en bestemt brukerkontekst, angir du brukerens e-post-ID i **Testbruker**-feltet i **Generelt**-fanen i Excel-parameterfilen.</span><span class="sxs-lookup"><span data-stu-id="9b327-488">To run test cases in a specific user context, enter the user's email ID in the **Test User** field on the **General** tab of the Excel parameter file.</span></span> <span data-ttu-id="9b327-489">I den nyeste versjonen av RSAT er oppsettet av feltene i Excel-parameterfilen oppdatert, men konseptet er uendret.</span><span class="sxs-lookup"><span data-stu-id="9b327-489">In the latest version of RSAT, the layout of the fields in the Excel parameter file has been updated, but the concept remains the same.</span></span>
    >
    > ![Testbruker-feltet](./media/setup_rsa_tool_83.png)

### <a name="validate-the-results"></a><span data-ttu-id="9b327-491">Validere resultatene</span><span class="sxs-lookup"><span data-stu-id="9b327-491">Validate the results</span></span>

- <span data-ttu-id="9b327-492">Velg **Kjør** for å kjøre testtilfellet på nytt, og kontroller at testtilfellet har bestått.</span><span class="sxs-lookup"><span data-stu-id="9b327-492">Select **Run** to rerun the test case, and verify that the test case has passed.</span></span> <span data-ttu-id="9b327-493">Du kan vise testresultatene som beskrevet i delen [Vise testresultatene](#view-the-test-results).</span><span class="sxs-lookup"><span data-stu-id="9b327-493">You can view the test results as described in the [View the test results](#view-the-test-results) section.</span></span>

    ![Resultat-feltet satt til Bestått](./media/setup_rsa_tool_84.png)

### <a name="chaining-of-test-cases"></a><span data-ttu-id="9b327-495">Kjeding av testtilfeller</span><span class="sxs-lookup"><span data-stu-id="9b327-495">Chaining of test cases</span></span>

<span data-ttu-id="9b327-496">En nøkkelfunksjon i RSAT er kjeding av testtilfeller (det vil si funksjonen som gjør at en test kan sende verdier til andre tester).</span><span class="sxs-lookup"><span data-stu-id="9b327-496">One key feature of RSAT is the chaining of test cases (that is, the capability of a test to pass values to other tests).</span></span> <span data-ttu-id="9b327-497">Testtilfeller kjøres i henhold til den definerte rekkefølgen i Azure DevOps-testplanen.</span><span class="sxs-lookup"><span data-stu-id="9b327-497">Test cases are run according to their defined order in the Azure DevOps test plan.</span></span> <span data-ttu-id="9b327-498">(Denne rekkefølgen kan også oppdateres i selve testverktøyet.) Hvis du vil sende variabler fra ett testtilfelle til et annet, er det derfor svært viktig at testene er i riktig rekkefølge.</span><span class="sxs-lookup"><span data-stu-id="9b327-498">(This order can also be updated in the test tool itself.) Therefore, if you want to pass variables from one test case to another, it's very important that the tests be in the correct order.</span></span>

<span data-ttu-id="9b327-499">I denne delen skal du opprette en lagret variabel i det første testtilfellet, opprette et andre testtilfelle og sende den lagrede variabelen fra det første til det andre testtilfellet.</span><span class="sxs-lookup"><span data-stu-id="9b327-499">In this section, you will create a saved variable in the first test case, create a second test case, and pass the saved variable from the first test case to the second test case.</span></span> <span data-ttu-id="9b327-500">Du skal deretter kjøre testtilfellen som et kjedet testtilfelle i RSAT.</span><span class="sxs-lookup"><span data-stu-id="9b327-500">You will then run the test cases as a chained test case in RSAT.</span></span>

#### <a name="modify-an-existing-task-recording-to-create-a-saved-variable"></a><span data-ttu-id="9b327-501">Endre et eksisterende oppgaveopptak for å opprette en lagret variabel</span><span class="sxs-lookup"><span data-stu-id="9b327-501">Modify an existing task recording to create a saved variable</span></span>

1. <span data-ttu-id="9b327-502">Start klienten.</span><span class="sxs-lookup"><span data-stu-id="9b327-502">Open the client.</span></span>
2. <span data-ttu-id="9b327-503">Velg **Innstillinger**-knappen (tannhjulsymbolet), og velg deretter **Oppgaveopptaker**.</span><span class="sxs-lookup"><span data-stu-id="9b327-503">Select the **Settings** button (the gear symbol), and then select **Task recorder**.</span></span>
3. <span data-ttu-id="9b327-504">Velg **Rediger opptak**.</span><span class="sxs-lookup"><span data-stu-id="9b327-504">Select **Edit Recording**.</span></span>

    ![Rediger opptak-knappen](./media/setup_rsa_tool_85.png)

4. <span data-ttu-id="9b327-506">Velg **Åpne fra Lifecycle Services**.</span><span class="sxs-lookup"><span data-stu-id="9b327-506">Select **Open from Lifecycle Services**.</span></span>

    ![Knappen Åpne fra Lifecycle Services](./media/setup_rsa_tool_86.png)

5. <span data-ttu-id="9b327-508">Velg **Velg Lifecycle Services-biblioteket**.</span><span class="sxs-lookup"><span data-stu-id="9b327-508">Select **Select the Lifecycle Services library**.</span></span>

    ![Knappen Velg Lifecycle Services-biblioteket](./media/setup_rsa_tool_87.png)

    <span data-ttu-id="9b327-510">BPM-biblioteker lastes inn fra LCS.</span><span class="sxs-lookup"><span data-stu-id="9b327-510">BPM libraries are loaded from LCS.</span></span>

    ![Fremdriftsindikator](./media/setup_rsa_tool_88.png)

6. <span data-ttu-id="9b327-512">Etter at BPM-bibliotekene er lastet inn fra LCS, velger du BPM-biblioteket **RSAT** og forretningsprosessen **Opprett et nytt produkt** som oppgaveopptaket var knyttet til.</span><span class="sxs-lookup"><span data-stu-id="9b327-512">After the BPM libraries are loaded from LCS, select the **RSAT** BPM library and the **Create a new product** business process that the task recording was associated with.</span></span> <span data-ttu-id="9b327-513">Velg deretter **OK**.</span><span class="sxs-lookup"><span data-stu-id="9b327-513">Then select **OK**.</span></span>

    ![Velge et BPM-bibliotek og en forretningsprosess](./media/setup_rsa_tool_89.png)

7. <span data-ttu-id="9b327-515">Navnet på det aktuelle oppgaveopptaket angis i feltet **Navn på opptak**.</span><span class="sxs-lookup"><span data-stu-id="9b327-515">The name of the appropriate task recording is entered in the **Recording name** field.</span></span> <span data-ttu-id="9b327-516">Velg **Start**.</span><span class="sxs-lookup"><span data-stu-id="9b327-516">Select **Start**.</span></span>

    ![Navnet på oppgaveopptaket i feltet Navn på opptak](./media/setup_rsa_tool_90.png)

8. <span data-ttu-id="9b327-518">Gå til **Behandling av produktinformasjon \> Produkter**, og velg **Ny** for å åpne siden der det opprinnelige oppgaveopptaket, **Opprett et produkt**, ble tatt opp.</span><span class="sxs-lookup"><span data-stu-id="9b327-518">Go to **Product information management \> Products**, and select **New** to open the page where the original task recording, **Create a product**, was recorded.</span></span>
9. <span data-ttu-id="9b327-519">Velg **Sett inn trinn**.</span><span class="sxs-lookup"><span data-stu-id="9b327-519">Select **Insert step**.</span></span>

    > [!NOTE]
    > <span data-ttu-id="9b327-520">Det nye trinnet settes inn **etter** trinnet du valgte i ruten.</span><span class="sxs-lookup"><span data-stu-id="9b327-520">The new step is inserted **after** the step that you selected in the pane.</span></span>

    ![Knappen Sett inn trinn](./media/setup_rsa_tool_91.png)

10. <span data-ttu-id="9b327-522">Høyreklikk på **Produktnummer**-feltet, og velg deretter **Oppgaveopptaker \> Kopier**.</span><span class="sxs-lookup"><span data-stu-id="9b327-522">Right-click the **Product number** field, and then select **Task recorder \> Copy**.</span></span>

    ![Kopier-kommandoen](./media/setup_rsa_tool_92.png)

11. <span data-ttu-id="9b327-524">Det legges til et nytt trinn i ruten.</span><span class="sxs-lookup"><span data-stu-id="9b327-524">A new step is added in the pane.</span></span> <span data-ttu-id="9b327-525">Noter verdien i **Produktnummer**-feltet siden du får behov for det senere.</span><span class="sxs-lookup"><span data-stu-id="9b327-525">Make a note of the value in the **Product number** field, because you will need it later.</span></span>

    ![Nytt trinn lagt til](./media/setup_rsa_tool_93.png)

12. <span data-ttu-id="9b327-527">Velg **Redigeringen er fullført**.</span><span class="sxs-lookup"><span data-stu-id="9b327-527">Select **Done editing**.</span></span>
13. <span data-ttu-id="9b327-528">Velg **Lagre i Lifecycle Services**, og knytt det nye oppgaveopptaket til det samme BPM-biblioteket og den samme forretningsprosessen som det opprinnelige oppgaveopptaket var knyttet til.</span><span class="sxs-lookup"><span data-stu-id="9b327-528">Select **Save to Lifecycle Services**, and associate the new task recording with the same BPM library and business process that the original task recording was associated with.</span></span> <span data-ttu-id="9b327-529">Hvis du vil ha mer informasjon, kan du se delen [Opprette et oppgaveopptak og lagre det i BPM-biblioteket](#create-a-task-recording-and-save-it-to-the-bpm-library).</span><span class="sxs-lookup"><span data-stu-id="9b327-529">For more information, see the [Create a task recording and save it to the BPM library](#create-a-task-recording-and-save-it-to-the-bpm-library) section.</span></span>
14. <span data-ttu-id="9b327-530">Gå til BPM-biblioteket, og velg **Synkroniser testtilfeller** for å skrive over oppgaveopptaket som er knyttet til testtilfellet i Azure DevOps, slik det er beskrevet i delen [Teste synkroniseringen fra BPM til Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops).</span><span class="sxs-lookup"><span data-stu-id="9b327-530">Go to the BPM library, and select **Sync testcases** to overwrite the task recording that is attached to the test case in Azure DevOps, as described in the [Test the synchronization from BPM to Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops) section.</span></span>
15. <span data-ttu-id="9b327-531">Åpne RSAT, og velg **Last inn** for å laste inn alle testtilfellene i testserien på nytt.</span><span class="sxs-lookup"><span data-stu-id="9b327-531">Open RSAT, and select **Load** to reload all the test cases in the test suite.</span></span> <span data-ttu-id="9b327-532">Du må generere automatiserings- og parameterfilene for det aktuelle testtilfellet på nytt ved å velge testtilfellet og deretter **Ny \> Generer testkjøring og parameterfiler**, slik det er beskrevet i delen [Laste inn og kjøre testtilfeller](#load-and-run-test-cases).</span><span class="sxs-lookup"><span data-stu-id="9b327-532">You must regenerate the automation and parameter files for the appropriate test case by selecting the test case and then selecting **New \> Generate Test Execution and Parameter files**, as described in the [Load and run test cases](#load-and-run-test-cases) section.</span></span>

    > [!NOTE]
    > <span data-ttu-id="9b327-533">Hvis du lot Excel-parameterfilen være åpen, mislykkes ny generering.</span><span class="sxs-lookup"><span data-stu-id="9b327-533">If the Excel parameter file was left open, regeneration will fail.</span></span> <span data-ttu-id="9b327-534">Derfor må du kontrollere at Excel-parameterfilen for testtilfellet lukkes før du genererer den nye Excel-parameterfilen.</span><span class="sxs-lookup"><span data-stu-id="9b327-534">Therefore, make sure that the Excel parameter file for the test case is closed before you generate the new Excel parameter file.</span></span>

16. <span data-ttu-id="9b327-535">Velg **Rediger** for å åpne den nye Excel-parameterfilen.</span><span class="sxs-lookup"><span data-stu-id="9b327-535">Select **Edit** to open the new Excel parameter file.</span></span> <span data-ttu-id="9b327-536">En ny **Lagret variabel**-oppføring vises på linje 9.</span><span class="sxs-lookup"><span data-stu-id="9b327-536">You will see a new **Saved variable** entry on line 9.</span></span> <span data-ttu-id="9b327-537">Denne variabelen, **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}**, lagres i XML-filen for oppgaveopptaket og kan brukes i senere tester.</span><span class="sxs-lookup"><span data-stu-id="9b327-537">This variable, **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}**, is saved in the task recording's XML file and can be used in subsequent tests.</span></span>

    ![Lagret variabel-oppføringen](./media/setup_rsa_tool_94.png)

#### <a name="create-a-new-test-case"></a><span data-ttu-id="9b327-539">Opprette et nytt testtilfelle</span><span class="sxs-lookup"><span data-stu-id="9b327-539">Create a new test case</span></span>

1. <span data-ttu-id="9b327-540">Gå til BPM-biblioteket **RSAT**.</span><span class="sxs-lookup"><span data-stu-id="9b327-540">Go to the **RSAT** BPM library.</span></span>
2. <span data-ttu-id="9b327-541">Velg prosessen **Eksempel på støtteforretningsprosess**, og velg deretter **Redigeringsmodus** til høyre.</span><span class="sxs-lookup"><span data-stu-id="9b327-541">Select the **Sample Support Business Process** process, and then, on the right, select **Edit mode**.</span></span>
3. <span data-ttu-id="9b327-542">Endre verdien i **Navn**-feltet og **Beskrivelse**-feltet til **Frigi produkt**.</span><span class="sxs-lookup"><span data-stu-id="9b327-542">Change the value of both the **Name** field and the **Description** field to **Release a product**.</span></span> <span data-ttu-id="9b327-543">Velg deretter **Lagre**.</span><span class="sxs-lookup"><span data-stu-id="9b327-543">Then select **Save**.</span></span>

    ![Navnet og beskrivelsen er endret til Frigi produkt](./media/setup_rsa_tool_95.png)

#### <a name="create-a-new-task-recording-that-has-a-validate-function"></a><span data-ttu-id="9b327-545">Opprette et nytt oppgaveopptak som har en valideringsfunksjon</span><span class="sxs-lookup"><span data-stu-id="9b327-545">Create a new task recording that has a Validate function</span></span>

- <span data-ttu-id="9b327-546">Opprett et oppgaveopptak for å frigi produktet som ble opprettet tidligere, til den juridiske enheten USRT.</span><span class="sxs-lookup"><span data-stu-id="9b327-546">Create a task recording to release the product that was created earlier to the USRT legal entity.</span></span> <span data-ttu-id="9b327-547">Hvis du vil ha mer informasjon, kan du se delen [Opprette et oppgaveopptak og lagre det i BPM-biblioteket](#create-a-task-recording-and-save-it-to-the-bpm-library).</span><span class="sxs-lookup"><span data-stu-id="9b327-547">For more information, see the [Create a task recording and save it to the BPM library](#create-a-task-recording-and-save-it-to-the-bpm-library) section.</span></span>

    > [!NOTE]
    > <span data-ttu-id="9b327-548">Når det gjelder kjedede testtilfeller, anbefaler vi alltid at du finner eller filtrerer for posten du trenger, ved å *skrive inn verdien for feltet manuelt*.</span><span class="sxs-lookup"><span data-stu-id="9b327-548">For chained test cases, we always recommend that you find or filter for the record that you require by *manually typing the value of the field*.</span></span> <span data-ttu-id="9b327-549">På den måten kan verktøyet fastsette posten som handlingen må utføres mot, i det etterfølgende testtilfellet.</span><span class="sxs-lookup"><span data-stu-id="9b327-549">In that way, the tool can determine the record that the action must be taken against in the subsequent test case.</span></span>

    ![Nytt oppgaveopptak som har en valideringsfunksjon](./media/setup_rsa_tool_96.png)

    <span data-ttu-id="9b327-551">Som den foregående illustrasjonen viser, validerer du verdien i **Produktnummer**-feltet etter at produktet er funnet ved hjelp av hurtigfilteret, men før du velger **Frigi produkter**, for å kontrollere at produkt-ID-en er produkt-ID-en som ble opprettet tidligere.</span><span class="sxs-lookup"><span data-stu-id="9b327-551">As the preceding illustration shows, after the product is found by using the Quick Filter, but before you select **Release products**, you validate the value of the **Product number** field to make sure that the product ID is the product ID that was created earlier.</span></span> <span data-ttu-id="9b327-552">Du kan validere verdien ved å høyreklikke på **Produktnummer**-feltet og deretter velge **Oppgaveopptaker \> Valider \> Gjeldende verdi**.</span><span class="sxs-lookup"><span data-stu-id="9b327-552">To validate the value, right-click the **Product number** field, and then select **Task recorder \> Validate \> Current Value**.</span></span>

    ![Validere gjeldende verdi](./media/setup_rsa_tool_97.png)

#### <a name="save-the-task-recording-to-bpm"></a><span data-ttu-id="9b327-554">Lagre oppgaveopptaket i BPM</span><span class="sxs-lookup"><span data-stu-id="9b327-554">Save the task recording to BPM</span></span>

1. <span data-ttu-id="9b327-555">Etter at oppgaveopptaket er fullført, velger du **Lagre i Lifecycle Services**.</span><span class="sxs-lookup"><span data-stu-id="9b327-555">After the task recording is completed, select **Save to Lifecycle Services**.</span></span>

    ![Lagringsalternativer](./media/setup_rsa_tool_98.png)

2. <span data-ttu-id="9b327-557">Bibliotekinformasjon lastes inn fra LCS.</span><span class="sxs-lookup"><span data-stu-id="9b327-557">Library information is loaded from LCS.</span></span>

    ![Fremdriftsindikator](./media/setup_rsa_tool_99.png)

3. <span data-ttu-id="9b327-559">Velg BPM-biblioteket som oppgaveopptaket skal knyttes til.</span><span class="sxs-lookup"><span data-stu-id="9b327-559">Select the BPM library to associate the task recording with.</span></span> <span data-ttu-id="9b327-560">For denne opplæringen velger du BPM-biblioteket **RSAT** som ble opprettet tidligere, og forretningsprosessen **Frigi produkt** under det.</span><span class="sxs-lookup"><span data-stu-id="9b327-560">For this tutorial, select the **RSAT** BPM library that was created earlier and the **Release a product** business process under it.</span></span> <span data-ttu-id="9b327-561">Velg deretter **OK**.</span><span class="sxs-lookup"><span data-stu-id="9b327-561">Then select **OK**.</span></span>

#### <a name="sync-bpm-to-azure-devops"></a><span data-ttu-id="9b327-562">Synkronisere BPM med Azure DevOps</span><span class="sxs-lookup"><span data-stu-id="9b327-562">Sync BPM to Azure DevOps</span></span>

1. <span data-ttu-id="9b327-563">Gå til BPM-biblioteket, og åpne **RSAT**-biblioteket.</span><span class="sxs-lookup"><span data-stu-id="9b327-563">Go to the BPM library, and open the **RSAT** library.</span></span>
2. <span data-ttu-id="9b327-564">Velg **VSTS-synkronisering** og deretter **Synkroniser testtilfeller**.</span><span class="sxs-lookup"><span data-stu-id="9b327-564">Select **VSTS sync** and then **Sync test cases**.</span></span> <span data-ttu-id="9b327-565">Hvis du vil ha mer informasjon, kan du se delen [Teste synkroniseringen fra BPM til Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops).</span><span class="sxs-lookup"><span data-stu-id="9b327-565">For more information, see the [Test the synchronization from BPM to Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops) section.</span></span>

    <span data-ttu-id="9b327-566">Etter at synkroniseringen er fullført, vises det nye arbeidselementet og det tilsvarende testtilfellet for forretningsprosessen **Frigi produkt** i Azure DevOps i **Tavler \> Arbeidselementer**.</span><span class="sxs-lookup"><span data-stu-id="9b327-566">After the synchronization is completed, the new work item and the corresponding test case for the **Release a product** business process appear in Azure DevOps at **Boards \> Work Items**.</span></span>

#### <a name="add-the-new-test-case-to-the-existing-test-suite"></a><span data-ttu-id="9b327-567">Legge til det nye testtilfellet i den eksisterende testserien</span><span class="sxs-lookup"><span data-stu-id="9b327-567">Add the new test case to the existing test suite</span></span>

1. <span data-ttu-id="9b327-568">Gå til **Testplaner \> Testplaner**, og velg planen **RSAT-testplan**.</span><span class="sxs-lookup"><span data-stu-id="9b327-568">Go to **Test plans \> Test plans**, and select the **RSAT Test Plan** plan.</span></span>
2. <span data-ttu-id="9b327-569">Velg **Legg til eksisterende**.</span><span class="sxs-lookup"><span data-stu-id="9b327-569">Select **Add existing**.</span></span>
3. <span data-ttu-id="9b327-570">Velg **Kjør spørring** på siden **Legg til testtilfeller i serie**.</span><span class="sxs-lookup"><span data-stu-id="9b327-570">On the **Add test cases to suite** page, select **Run query**.</span></span>
4. <span data-ttu-id="9b327-571">Velg det nye testtilfellet som ble opprettet for **Frigi produkt**, og velg deretter **Legg til testtilfeller** i nedre høyre hjørne på siden (denne knappen vises ikke i illustrasjonen nedenfor).</span><span class="sxs-lookup"><span data-stu-id="9b327-571">Select the new test case that was created for **Release a product**, and then select **Add test cases** in the lower-right corner of the page (this button isn't shown in the following illustration).</span></span>

    ![Siden Legg til testtilfeller i serie](./media/setup_rsa_tool_100.png)

    <span data-ttu-id="9b327-573">Testserien har nå to testtilfeller.</span><span class="sxs-lookup"><span data-stu-id="9b327-573">The test suite now has two test cases.</span></span>

    ![To testtilfeller i testserien](./media/setup_rsa_tool_101.png)

#### <a name="load-test-cases-into-rsat"></a><span data-ttu-id="9b327-575">Laste inn testtilfeller i RSAT</span><span class="sxs-lookup"><span data-stu-id="9b327-575">Load test cases into RSAT</span></span>

1. <span data-ttu-id="9b327-576">Åpne RSAT, og velg **Last inn**.</span><span class="sxs-lookup"><span data-stu-id="9b327-576">Open RSAT, and select **Load**.</span></span>
2. <span data-ttu-id="9b327-577">Testtilfellene lastes inn, og du får en advarsel om at denne handlingen skriver over Excel-testdatafiler, og at lokale endringer går tapt.</span><span class="sxs-lookup"><span data-stu-id="9b327-577">The test cases are loaded, and you receive a warning that states, "This action will overwrite Excel test data files, local changes will be lost.</span></span> <span data-ttu-id="9b327-578">Du blir også spurt om du vil fortsette.</span><span class="sxs-lookup"><span data-stu-id="9b327-578">Do you want to proceed?"</span></span> <span data-ttu-id="9b327-579">Velg **Ja** for å oppdatere Excel-parameterfiler i det lokale systemet, men ikke Excel-parameterfilene som ble lastet opp til Azure DevOps.</span><span class="sxs-lookup"><span data-stu-id="9b327-579">Select **Yes** to update the Excel parameter files in the local system but not the Excel parameter files that were uploaded to Azure DevOps.</span></span>

    ![Advarselsmelding](./media/setup_rsa_tool_102.png)

    <span data-ttu-id="9b327-581">Begge testtilfellene lastes inn, sammen med Excel-parameterfilen for det første testtilfellet.</span><span class="sxs-lookup"><span data-stu-id="9b327-581">Both the test cases are loaded, together with the Excel parameter file for the first test case.</span></span> <span data-ttu-id="9b327-582">Siden du valgte **Last opp** i den siste kjøringen, hentes parameterfilene fra Azure DevOps.</span><span class="sxs-lookup"><span data-stu-id="9b327-582">Because you selected **Upload** in the last run, the parameter files are pulled from Azure DevOps.</span></span>

    ![Testtilfeller lastet inn](./media/setup_rsa_tool_103.png)

3. <span data-ttu-id="9b327-584">Velg bare det andre testtilfellet, og velg deretter **Ny \> Generer testkjøring og parameterfiler**.</span><span class="sxs-lookup"><span data-stu-id="9b327-584">Select only the second test case, and then select **New \> Generate test execution and parameter files**.</span></span>

#### <a name="edit-the-excel-parameter-file"></a><span data-ttu-id="9b327-585">Redigere Excel-parameterfilen</span><span class="sxs-lookup"><span data-stu-id="9b327-585">Edit the Excel parameter file</span></span>

1. <span data-ttu-id="9b327-586">Velg bare det andre testtilfellet, og velg deretter **Rediger** for å åpne den tilsvarende Excel-parameterfilen.</span><span class="sxs-lookup"><span data-stu-id="9b327-586">Select only the second test case, and then select **Edit** to open the corresponding Excel parameter file.</span></span>
2. <span data-ttu-id="9b327-587">Kopier den lagrede variabelen **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}** (se delen [Endre et eksisterende oppgaveopptak for å opprette en lagret variabel](#modify-an-existing-task-recording-to-create-a-saved-variable)) fra det første testtilfellet til alle feltene der produktnummeret brukes.</span><span class="sxs-lookup"><span data-stu-id="9b327-587">Copy the **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}** saved variable (see the [Modify an existing task recording to create a saved variable](#modify-an-existing-task-recording-to-create-a-saved-variable) section) from the first test case into all the fields where the product number is used.</span></span> <span data-ttu-id="9b327-588">I dette tilfellet kopierer du variabelen til feltene **Produktnummer** og **Valider produktnummer** i arket **EcoResProductListPage**.</span><span class="sxs-lookup"><span data-stu-id="9b327-588">In this case, you copy the variable into the **Product number** and **Validate Product number** fields on the **EcoResProductListPage** sheet.</span></span>

    ![Feltene Produktnummer og Valider produktnummer](./media/setup_rsa_tool_104.png)

    > [!NOTE]
    > <span data-ttu-id="9b327-590">Variabler kan bare sendes mellom tester i den samme testkjøringen.</span><span class="sxs-lookup"><span data-stu-id="9b327-590">Variables can be passed between tests only during the same test run.</span></span> <span data-ttu-id="9b327-591">Navnene på variablene må samsvare nøyaktig.</span><span class="sxs-lookup"><span data-stu-id="9b327-591">The names of the variables must match exactly.</span></span>

3. <span data-ttu-id="9b327-592">Velg **Lagre**, og lukk deretter Excel-arbeidsboken.</span><span class="sxs-lookup"><span data-stu-id="9b327-592">Select **Save**, and then close the Excel workbook.</span></span>
4. <span data-ttu-id="9b327-593">Velg **Last opp** for å lagre endringene du har gjort i Excel-parameterfilen.</span><span class="sxs-lookup"><span data-stu-id="9b327-593">Select **Upload** to save the changes that you made to the Excel parameter file.</span></span>

#### <a name="run-the-chained-test-cases"></a><span data-ttu-id="9b327-594">Kjøre kjedede testtilfeller</span><span class="sxs-lookup"><span data-stu-id="9b327-594">Run the chained test cases</span></span>

1. <span data-ttu-id="9b327-595">Velg begge testtilfellene, og velg deretter **Kjør**.</span><span class="sxs-lookup"><span data-stu-id="9b327-595">Select both the test cases, and then select **Run**.</span></span>
2. <span data-ttu-id="9b327-596">Kontroller at begge testtilfellene har bestått.</span><span class="sxs-lookup"><span data-stu-id="9b327-596">Verify that both test cases have passed.</span></span>
