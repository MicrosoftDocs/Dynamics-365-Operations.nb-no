...
<span data-ttu-id="c1d65-106">Dette emnet beskriver retningslinjen for beholdningsreservasjon som lar disse virksomhetene reservere spesifikke partier eller lisensalter, selv når produktene er assosiert med et "Parti under\[plassering\]"-reservasjonshierarki.</span><span class="sxs-lookup"><span data-stu-id="c1d65-106">This topic describes the inventory reservation policy that lets these businesses reserve specific batches or license plates, even when the products are associated with a "Batch-below\[location\]" reservation hierarchy.</span></span>
...
## <a name="example-scenario-batch-number-allocation"></a><span data-ttu-id="c1d65-164">Eksempelscenario: Tildeling av partinummer</span><span class="sxs-lookup"><span data-stu-id="c1d65-164">Example scenario: Batch number allocation</span></span>
...
1. <span data-ttu-id="c1d65-177">Angi de tre hoveddataparameterne for produktet ved å bruke disse verdiene:</span><span class="sxs-lookup"><span data-stu-id="c1d65-177">Set the product's three master data parameters by using these values:</span></span>
...
8. <span data-ttu-id="c1d65-244">Gjennomgå varens lagertransaksjoner som er knyttet til salgsordrelinjereservasjonen.</span><span class="sxs-lookup"><span data-stu-id="c1d65-244">Review the item's inventory transactions that are related to the sales order line reservation.</span></span>
    ...
    - <span data-ttu-id="c1d65-247">En transaksjon der **Referanse**-feltet er satt til **Ordreigangsatt reservasjon** og **Utsted**-feltet er satt til **Reservert fysisk**, representerer ordrelinjereservasjonen for det spesifikke partiet og alle beholdningsdimensjonene over det.</span><span class="sxs-lookup"><span data-stu-id="c1d65-247">A transaction where the **Reference** field is set to **Order-committed reservation** and the **Issue** field is set to **Reserved physical** represents the order line reservation for the specific batch and all inventory dimensions above it.</span></span> <span data-ttu-id="c1d65-248">I henhold til varens beholdningsreservasjonshierarki er disse dimensjonene partinummer og plassering.</span><span class="sxs-lookup"><span data-stu-id="c1d65-248">According to the item's inventory reservation hierarchy, those dimensions are batch number and location.</span></span> <span data-ttu-id="c1d65-249">I dette eksempelet er plasseringen **Bulk-001**.</span><span class="sxs-lookup"><span data-stu-id="c1d65-249">In this example, the location is **Bulk-001**.</span></span>
...
## <a name="flexible-license-plate-reservation"></a><span data-ttu-id="c1d65-273">Fleksibel nummerskiltreservering</span><span class="sxs-lookup"><span data-stu-id="c1d65-273">Flexible license plate reservation</span></span>
...
1. <span data-ttu-id="c1d65-286">**Funksjonsnavn:** *Fleksibel dimensjonsreservasjonspolicy for lagernivå*</span><span class="sxs-lookup"><span data-stu-id="c1d65-286">**Feature name:** *Flexible warehouse-level dimension reservation*</span></span>
...
- <span data-ttu-id="c1d65-298">Referanse (bare *Salgsordre*-verdien støttes.)</span><span class="sxs-lookup"><span data-stu-id="c1d65-298">Reference (Only the *Sales order* value is supported.)</span></span>
...
## <a name="example-scenario-set-up-and-process-an-order-committed-license-plate-reservation"></a><span data-ttu-id="c1d65-312">Eksempelscenario: Definere og behandle en ordreigangsatt nummerskiltreservering</span><span class="sxs-lookup"><span data-stu-id="c1d65-312">Example scenario: Set up and process an order-committed license plate reservation</span></span>
...
1. <span data-ttu-id="c1d65-346">Juster lagerbeholdningen for de nye varene slik det er angitt i følgende tabell.</span><span class="sxs-lookup"><span data-stu-id="c1d65-346">Adjust the on-hand inventory of the new items as specified in the following table.</span></span>
    ...
    | <span data-ttu-id="c1d65-347">Artikkel</span><span class="sxs-lookup"><span data-stu-id="c1d65-347">Item</span></span>  | <span data-ttu-id="c1d65-348">Lager</span><span class="sxs-lookup"><span data-stu-id="c1d65-348">Warehouse</span></span> | <span data-ttu-id="c1d65-349">Lokasjon</span><span class="sxs-lookup"><span data-stu-id="c1d65-349">Location</span></span> | <span data-ttu-id="c1d65-350">Nummerskilt</span><span class="sxs-lookup"><span data-stu-id="c1d65-350">License plate</span></span> | <span data-ttu-id="c1d65-351">Antall</span><span class="sxs-lookup"><span data-stu-id="c1d65-351">Quantity</span></span> |
    |-------|-----------|----------|---------------|----------|
...
1. <span data-ttu-id="c1d65-383">Legg til en ny salgsordrelinje som har følgende innstillinger:</span><span class="sxs-lookup"><span data-stu-id="c1d65-383">Add a second sales order line that has the following settings:</span></span>
    ...
    - <span data-ttu-id="c1d65-384">**Varenummer:** *Vare 2*</span><span class="sxs-lookup"><span data-stu-id="c1d65-384">**Item number:** *Item2*</span></span>
...
1. <span data-ttu-id="c1d65-396">Hvis du vil reservere en vare på et bestemt nummerskilt i Excel-tillegget, velger du **Ny** for å legge til en reservasjonslinje, og angir deretter følgende verdier:</span><span class="sxs-lookup"><span data-stu-id="c1d65-396">To reserve an item on a specific license plate, in the Excel add-in, select **New** to add a reservation line, and then set the following values:</span></span>
    ...
    - <span data-ttu-id="c1d65-397">**Parti-ID:** Angi verdien for **Parti-ID** som du fant for salgsordrelinjen for *Vare 1*.</span><span class="sxs-lookup"><span data-stu-id="c1d65-397">**Lot ID:** Enter the **Lot ID** value that you found for the sales order line for *Item1*.</span></span>
...
1. <span data-ttu-id="c1d65-409">Hvis du vil se gjennom lagertransaksjoner som er knyttet til salgsordrelinjereservasjonen, går du til **Salgsordrelinjer**-hurtigfanen på **Lager**-menyen og velger **Vis \> Transaksjoner**.</span><span class="sxs-lookup"><span data-stu-id="c1d65-409">To review inventory transactions that are related to the sales order line reservation, on the **Sales order lines** FastTab, on the **Inventory** menu, select **View \> Transactions**.</span></span> <span data-ttu-id="c1d65-410">Legg merke til at det er to transaksjoner som er knyttet til reserveringen: én der **Referanse**-feltet er satt til *Salgsordre*, og én der **Referanse**-feltet er satt til *Ordreigangsatt reservasjon*.</span><span class="sxs-lookup"><span data-stu-id="c1d65-410">Notice that there are two transactions that are related to the reservation: one where the **Reference** field is set to *Sales order* and one where the **Reference** field is set to *Order-committed reservation*.</span></span>

    > [!NOTE]
    > <span data-ttu-id="c1d65-411">En transaksjon der **Referanse**-feltet er satt til *Salgsordre* , representerer ordrelinjereservasjonen for beholdningsdimensjonene som er over **Plassering**-nivået (anlegg, lager og inventarstatus).</span><span class="sxs-lookup"><span data-stu-id="c1d65-411">A transaction where the **Reference** field is set to *Sales order* represents the order line reservation for inventory dimensions that are above the **Location** level (site, warehouse, and inventory status).</span></span> <span data-ttu-id="c1d65-412">En transaksjon der **Referanse**-feltet er angitt til *Ordreigangsatt reservasjon*, representerer ordrelinjereservasjonen for det bestemte nummerskiltet og lokasjonen.</span><span class="sxs-lookup"><span data-stu-id="c1d65-412">A transaction where the **Reference** field is set to *Order-committed reservation* represents the order line reservation for the specific license plate and location.</span></span>

1. <span data-ttu-id="c1d65-413">Du kan frigi salgsordren ved å gå til handlingsruten og **Lager**-fanen i **Handlinger**-gruppen og velge **Frigi til lager**.</span><span class="sxs-lookup"><span data-stu-id="c1d65-413">To release the sales order, on the Action Pane, on the **Warehouse** tab, in the **Actions** group, select **Release to warehouse**.</span></span>

### <a name="review-and-process-warehouse-work-with-order-committed-license-plates-assigned"></a><span data-ttu-id="c1d65-414">Gå gjennom og behandle lagerarbeidet med ordreigangsatte nummerskilt tilordnet</span><span class="sxs-lookup"><span data-stu-id="c1d65-414">Review and process warehouse work with order-committed license plates assigned</span></span>

1. <span data-ttu-id="c1d65-415">På **Salgsordrelinjer**-fanen, på **Lager**-menyen, velger du **Arbeidsdetaljer**.</span><span class="sxs-lookup"><span data-stu-id="c1d65-415">On the **Sales order lines** FastTab, on the **Warehouse** menu, select **Work details**.</span></span>

    <span data-ttu-id="c1d65-416">Når reservasjon er utført for et bestemt parti, bruker ikke systemet lokasjonsdirektiver når det oppretter arbeidet for salgsordren som bruker nummerskiltreservering.</span><span class="sxs-lookup"><span data-stu-id="c1d65-416">As when reservation is done for a specific batch, the system doesn't use location directives when it creates the work for the sales order that uses license plate reservation.</span></span> <span data-ttu-id="c1d65-417">Siden den ordreigangsatte reservasjonen angir alle lagerdimensjonene, inkludert lokasjonen, trenger ikke lokasjonsdirektiver å brukes, fordi disse lagerdimensjonene akkurat ble angitt i arbeidet.</span><span class="sxs-lookup"><span data-stu-id="c1d65-417">Because the order-committed reservation specifies all the inventory dimensions, including the location, location directives don't have to be used, because those inventory dimensions are just entered in the work.</span></span> <span data-ttu-id="c1d65-418">De vises i delen **Fra lagerdimensjoner** på siden **Arbeidslagertransaksjoner**.</span><span class="sxs-lookup"><span data-stu-id="c1d65-418">They are shown in the **From inventory dimensions** section on the **Work inventory transactions** page.</span></span>

    > [!NOTE]
    > <span data-ttu-id="c1d65-419">Etter at arbeidet er opprettet, blir varens beholdningstransaksjon der **Referanse**-feltet er satt til *Ordreigangsatt reservasjon*, fjernet.</span><span class="sxs-lookup"><span data-stu-id="c1d65-419">After the work is created, the item's inventory transaction where the **Reference** field is set to *Order-committed reservation* is removed.</span></span> <span data-ttu-id="c1d65-420">Beholdningstransaksjonen der **Referanse**-feltet er satt til *Arbeid*, inneholder nå den fysiske reservasjonen for alle beholdningsdimensjoner for antallet.</span><span class="sxs-lookup"><span data-stu-id="c1d65-420">The inventory transaction where the **Reference** field is set to *Work* now holds the physical reservation for all the quantity's inventory dimensions.</span></span>

1. <span data-ttu-id="c1d65-421">Fullfør plukkingen på mobilenheten, og plasser arbeidet ved hjelp av et menyelement der det er merket av for **Behandle etter nummerskilt**.</span><span class="sxs-lookup"><span data-stu-id="c1d65-421">On the mobile device, finish picking and putting the work by using a menu item where the **Handle by license plate** check box is selected.</span></span>

    > [!NOTE]
    > <span data-ttu-id="c1d65-422">Funksjonaliteten **Behandle etter nummerskilt** hjelper deg med å behandle hele nummerskiltet.</span><span class="sxs-lookup"><span data-stu-id="c1d65-422">The **Handle by license plate** functionality helps you process the whole license plate.</span></span> <span data-ttu-id="c1d65-423">Hvis du må behandle deler av nummerskiltet, kan du ikke bruke denne funksjonaliteten.</span><span class="sxs-lookup"><span data-stu-id="c1d65-423">If you must process part of the license plate, you can't use this functionality.</span></span>
    >
    > <span data-ttu-id="c1d65-424">Det anbefales at du har separat arbeid generert for hvert nummerskilt.</span><span class="sxs-lookup"><span data-stu-id="c1d65-424">We recommend that you have separate work generated for each license plate.</span></span> <span data-ttu-id="c1d65-425">Du oppnår dette resultatet ved å bruke funksjonen **Arbeidshodeskift** på **Arbeidsmal**-siden.</span><span class="sxs-lookup"><span data-stu-id="c1d65-425">To achieve this result, use the **Work header breaks** feature on the **Work template** page.</span></span>

    <span data-ttu-id="c1d65-426">Nummerskiltet *LP02* plukkes nå for salgsordrelinjer og plasseres på *Rampedør*-lokasjonen.</span><span class="sxs-lookup"><span data-stu-id="c1d65-426">License plate *LP02* is now picked for sales order lines and put to the *Baydoor* location.</span></span> <span data-ttu-id="c1d65-427">På dette tidspunktet er det klart til å lastes og sendes til kunden.</span><span class="sxs-lookup"><span data-stu-id="c1d65-427">At this point, it's ready to be loaded and dispatched to the customer.</span></span>

## <a name="exception-handling-of-warehouse-work-that-has-order-committed-batch-numbers"></a><span data-ttu-id="c1d65-428">Unntakshåndtering av lagerarbeid som har ordreigangsatte partinumre</span><span class="sxs-lookup"><span data-stu-id="c1d65-428">Exception handling of warehouse work that has order-committed batch numbers</span></span>

<span data-ttu-id="c1d65-429">Lagerarbeid for plukking av ordreigangsatte partinumre er underlagt samme standard lagerunntakshåndtering og -handlinger som vanlig arbeid.</span><span class="sxs-lookup"><span data-stu-id="c1d65-429">Warehouse work for picking order-committed batch numbers is subject to the same standard warehouse exception handling and actions as regular work.</span></span> <span data-ttu-id="c1d65-430">Generelt kan det åpne arbeidet eller arbeidslinjen avbrytes. De kan avbrytes fordi en brukerplassering er full, det kan mangle tilstrekkelig antall, og de kan oppdateres på grunn av en bevegelse.</span><span class="sxs-lookup"><span data-stu-id="c1d65-430">In general, the open work or work line can be canceled, it can be interrupted because a user location is full, it can be short-picked, and it can be updated because of a movement.</span></span> <span data-ttu-id="c1d65-431">Tilsvarende kan den valgte kvantiteten arbeid som allerede er fullført, reduseres, eller arbeidet kan reverseres.</span><span class="sxs-lookup"><span data-stu-id="c1d65-431">Likewise, the picked quantity of work that has already been completed can be reduced, or the work can be reversed.</span></span>

<span data-ttu-id="c1d65-432">Følgende nøkkelregel brukes for alle disse unntakshåndteringshandlingene: Partinummeret som var reservert for kunden, kan aldri erstattes med et annet partinummer, men lagringsdimensjonene (plassering og nummerskilt) kan endres gjennom enten en manuell oppdatering utført av brukeren eller en automatisk oppdatering utført av systemet.</span><span class="sxs-lookup"><span data-stu-id="c1d65-432">The following key rule is applied to all these exception handling actions: the batch number that was reserved for the customer can never be replaced with a different batch number, but its storage dimensions (location and license plate) can be changed through either a manual update by the user or an automatic update by the system.</span></span> <span data-ttu-id="c1d65-433">Den automatiske oppdateringen er basert på den samme tilfeldige tilordningen av lagringsdimensjoner som gjaldt da et spesifikt parti ble reservert automatisk, men ingen lagringsdimensjoner ble spesifisert.</span><span class="sxs-lookup"><span data-stu-id="c1d65-433">The automatic update is based on the same random assignment of storage dimensions that applied when a specific batch was automatically reserved but no storage dimensions were specified.</span></span>

### <a name="example-scenario"></a><span data-ttu-id="c1d65-434">Eksempelscenario</span><span class="sxs-lookup"><span data-stu-id="c1d65-434">Example scenario</span></span>

<span data-ttu-id="c1d65-435">Et eksempel på dette scenarioet er en situasjon der tidligere avsluttet arbeid plukkes fra hverandre ved å benytte **Reduser plukket kvantitet**-funksjonen.</span><span class="sxs-lookup"><span data-stu-id="c1d65-435">An example of this scenario is a situation where previously completed work is being unpicked by using the **Reduce picked quantity** function.</span></span> <span data-ttu-id="c1d65-436">Dette eksemplet forutsetter at du allerede har fullført trinnene som er beskrevet i [Eksempelscenario: Tildeling av partinummer](#Example-batch-allocation).</span><span class="sxs-lookup"><span data-stu-id="c1d65-436">This example assumes that you've already completed the steps that are described in [Example scenario: Batch number allocation](#Example-batch-allocation).</span></span> <span data-ttu-id="c1d65-437">Den fortsetter fra dette eksemplet.</span><span class="sxs-lookup"><span data-stu-id="c1d65-437">It continues from that example.</span></span>

1. <span data-ttu-id="c1d65-438">Gå til **Lagerstyring** \> **Belastninger** \> **Aktive belastninger**.</span><span class="sxs-lookup"><span data-stu-id="c1d65-438">Go to **Warehouse management** \> **Loads** \> **Active loads**.</span></span>
2. <span data-ttu-id="c1d65-439">Velg belastningen som ble opprettet i forbindelse med forsendelsen av salgsordren.</span><span class="sxs-lookup"><span data-stu-id="c1d65-439">Select the load that was created in connection with the shipment of your sales order.</span></span>
3. <span data-ttu-id="c1d65-440">Gå til **Last inn ordrelinjer**-hurtigfanen og velg **Reduser plukket kvantitet**.</span><span class="sxs-lookup"><span data-stu-id="c1d65-440">From the **Load order lines** FastTab, select **Reduce picked quantity**.</span></span>
4. <span data-ttu-id="c1d65-441">Gå til **Reduser plukket kvantitet**-siden i **Flytt til plassering**-feltet og velg **FL-001**.</span><span class="sxs-lookup"><span data-stu-id="c1d65-441">On the **Reduce picked quantity** page, in the **Move to location** field, select **FL-001**.</span></span>
5. <span data-ttu-id="c1d65-442">I **Flytt til nummerskilt**-feltet velger du **LP33**.</span><span class="sxs-lookup"><span data-stu-id="c1d65-442">In the **Move to license plate** field, select **LP33**.</span></span>
6. <span data-ttu-id="c1d65-443">I rutenettet, i **Beholdningskvantitet som skal plukkes vekk**-feltet, angir du **10**.</span><span class="sxs-lookup"><span data-stu-id="c1d65-443">In the grid, in the **Inventory quantity to unpick** field, enter **10**.</span></span>
7. <span data-ttu-id="c1d65-444">Velg **OK**.</span><span class="sxs-lookup"><span data-stu-id="c1d65-444">Select **OK**.</span></span>

<span data-ttu-id="c1d65-445">Her er resultatene av handlingen med vekkplukking:</span><span class="sxs-lookup"><span data-stu-id="c1d65-445">Here are the results of the unpicking action:</span></span>

- <span data-ttu-id="c1d65-446">Statusen til det tidligere lukkede verket er satt til **Avbrutt**.</span><span class="sxs-lookup"><span data-stu-id="c1d65-446">The status of the previously closed work is set to **Canceled**.</span></span>
- <span data-ttu-id="c1d65-447">Nytt arbeid av **Beholdningsbevegelse**-typen opprettes for det vekkplukkede antallet **10** for partinummer **B11**.</span><span class="sxs-lookup"><span data-stu-id="c1d65-447">New work of the **Inventory movement** type is created for the unpicked quantity of **10** for batch number **B11**.</span></span> <span data-ttu-id="c1d65-448">Dette arbeidet representerer bevegelsen fra **Båsdør**-plasseringen til nummerskiltet **LP33** på plasseringen **FL-001**.</span><span class="sxs-lookup"><span data-stu-id="c1d65-448">This work represents the movement from the **Baydoor** location to license plate **LP33** in location **FL-001**.</span></span> <span data-ttu-id="c1d65-449">Status er satt til **Lukket**.</span><span class="sxs-lookup"><span data-stu-id="c1d65-449">The status is set to **Closed**.</span></span>
- <span data-ttu-id="c1d65-450">Systemet reserverer partinummeret som opprinnelig ble bestilt, på nytt og tilordner plasseringen og nummerskilt-ID-er.</span><span class="sxs-lookup"><span data-stu-id="c1d65-450">The system re-reserves the batch number that was originally ordered, and assigns the location and license plate IDs.</span></span> <span data-ttu-id="c1d65-451">(Denne prosessen tilsvarer kjøring av **Reserver linje**-funksjonen for ordrelinjen for et gitt partinummer).</span><span class="sxs-lookup"><span data-stu-id="c1d65-451">(This process is equivalent to running the **Reserve line** function for the order line for a given batch number).</span></span> <span data-ttu-id="c1d65-452">Som et resultat vises parti **B11** som bundet på **Partinumre bundet til kildelinje**-hurtigfanen på **Partireservasjon**-siden, og **Reservasjon**-feltet inneholder antallet **10** for partinummer **B11**.</span><span class="sxs-lookup"><span data-stu-id="c1d65-452">As a result, batch **B11** is shown as committed on the **Batch numbers committed to source line** FastTab of the **Batch reservation** page, and the **Reservation** field contains a quantity of **10** for batch number **B11**.</span></span> <span data-ttu-id="c1d65-453">I tillegg er **Plassering**-feltet satt til **FL-001**, og **Nummerskilt**-feltet er satt til **LP11**.</span><span class="sxs-lookup"><span data-stu-id="c1d65-453">Additionally, the **Location** field is set to **FL-001**, and the **License plate** field is set to **LP11**.</span></span> <span data-ttu-id="c1d65-454">(Du kan legge til disse feltene i rutenettet hvis de ikke er synlige.)</span><span class="sxs-lookup"><span data-stu-id="c1d65-454">(You can add these fields to the grid if they aren't visible.)</span></span>

<span data-ttu-id="c1d65-455">Følgende tabeller inneholder en oversikt som viser hvordan systemet håndterer ordreigangsatt partireservasjon for bestemte lagerhandlinger.</span><span class="sxs-lookup"><span data-stu-id="c1d65-455">The following tables provide an overview that shows how the system handles order-committed batch reservation for specific warehouse actions.</span></span> <span data-ttu-id="c1d65-456">Hvis du vil tolke innholdet i tabellene, må du anta at hver lagerhandling kjøres i konteksten av eksisterende lagerarbeid som stammer fra en ordreigangsatt partireservasjon, eller at utførelsen av hver lagerhandling påvirker arbeid av denne typen.</span><span class="sxs-lookup"><span data-stu-id="c1d65-456">To interpret the content in the tables, assume that each warehouse action is run in the context of existing warehouse work that originates from an order-committed batch reservation, or that execution of each warehouse action affects work of that type.</span></span>

> [!NOTE]
> <span data-ttu-id="c1d65-457">I disse tabellene angir "Partikvantitet er tilgjengelig"-kolonnen om en partikvantitet er tilgjengelig, i tillegg til kvantiteten som allerede er reservert for gjeldende ordreigangsatte reservasjoner, eller som allerede er reservert av lagerarbeidet som stammer fra reservasjoner av denne typen.</span><span class="sxs-lookup"><span data-stu-id="c1d65-457">In these tables, the "Batch quantity is available" column indicates whether a batch quantity is available in addition to the quantity that is either already reserved for the current order-committed reservations or already reserved by the warehouse work that originates from reservations of that type.</span></span>

#### <a name="override-the-pick-location-on-the-open-work"></a><span data-ttu-id="c1d65-458">Overstyr plukkplasseringen for det åpne arbeidet</span><span class="sxs-lookup"><span data-stu-id="c1d65-458">Override the pick location on the open work</span></span>

<table>
<thead>
<tr>
<th><span data-ttu-id="c1d65-459">Nøkkeloppsettsparameter</span><span class="sxs-lookup"><span data-stu-id="c1d65-459">Key setup parameter</span></span></th>
<th><span data-ttu-id="c1d65-460">Partikvantitet er tilgjengelig</span><span class="sxs-lookup"><span data-stu-id="c1d65-460">Batch quantity is available</span></span></th>
<th><span data-ttu-id="c1d65-461">Nøkkelbrukertrinn</span><span class="sxs-lookup"><span data-stu-id="c1d65-461">Key user steps</span></span></th>
<th><span data-ttu-id="c1d65-462">Lagerarbeid</span><span class="sxs-lookup"><span data-stu-id="c1d65-462">Warehouse work</span></span></th>
<th><span data-ttu-id="c1d65-463">Ordreigangsatt partireservasjon</span><span class="sxs-lookup"><span data-stu-id="c1d65-463">Order-committed batch reservation</span></span></th>
</tr>
</thead>
<tbody>
<tr>
<td rowspan='2'><span data-ttu-id="c1d65-464"><strong>Tillat overstyring av plukkplassering</strong>-alternativet er aktivert for arbeideren.</span><span class="sxs-lookup"><span data-stu-id="c1d65-464">The <strong>Allow pick location override</strong> option is enabled on the worker.</span></span></td>
<td><span data-ttu-id="c1d65-465">Ja</span><span class="sxs-lookup"><span data-stu-id="c1d65-465">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="c1d65-466">Velg <strong>Overstyr plassering</strong>-menyelementet på lagerappen når du begynner plukkarbeid.</span><span class="sxs-lookup"><span data-stu-id="c1d65-466">Select the <strong>Override location</strong> menu item on the warehouse app when you start picking work.</span></span></li>
<li><span data-ttu-id="c1d65-467">Velg <strong>Foreslå</strong>.</span><span class="sxs-lookup"><span data-stu-id="c1d65-467">Select <strong>Suggest</strong>.</span></span></li>
<li><span data-ttu-id="c1d65-468">Bekreft den nye plasseringen som foreslås, basert på tilgjengeligheten av partikvantitet.</span><span class="sxs-lookup"><span data-stu-id="c1d65-468">Confirm the new location that is suggested based on batch quantity availability.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="c1d65-469">For gjeldende arbeid oppstår det følgende handlinger:</span><span class="sxs-lookup"><span data-stu-id="c1d65-469">On the current work, the following actions occur:</span></span>
<ul>
<li><span data-ttu-id="c1d65-470">Plasseringen på plukklinjen oppdateres til den nye plasseringen.</span><span class="sxs-lookup"><span data-stu-id="c1d65-470">The location on the pick line is updated to the new location.</span></span> <span data-ttu-id="c1d65-471">(Hvis plasseringen er navneskiltstyrt, tilordnes et tilfeldig nummerskilt til arbeidslagertransaksjonen, og arbeideren kan plukke et hvilket som helst nummerskilt som har tilgjengelig kvantitet.)</span><span class="sxs-lookup"><span data-stu-id="c1d65-471">(If the location is license plate–controlled, a random license plate is assigned to the work inventory transaction, and the worker can pick from any license plate that has available quantity.)</span></span></li>
<li><span data-ttu-id="c1d65-472">Hvis kvantiteten blir funnet for mer enn ett nummerskilt på den nye plasseringen, blir den opprinnelige plukklinjen delt opp i flere linjer for å matche hvert nummerskilt.</span><span class="sxs-lookup"><span data-stu-id="c1d65-472">If the quantity is found on more than one license plate in the new location, the original pick line is split into multiple lines to match each license plate.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="c1d65-473">Gjelder ikke</span><span class="sxs-lookup"><span data-stu-id="c1d65-473">Not applicable</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="c1d65-474">Nei</span><span class="sxs-lookup"><span data-stu-id="c1d65-474">No</span></span></td>
<td>
<ol>
<li><span data-ttu-id="c1d65-475">Velg <strong>Overstyr plassering</strong>-menyelementet på lagerappen når du begynner plukkarbeid.</span><span class="sxs-lookup"><span data-stu-id="c1d65-475">Select the <strong>Override location</strong> menu item on the warehouse app when you start picking work.</span></span></li>
<li><span data-ttu-id="c1d65-476">Angi en plassering manuelt.</span><span class="sxs-lookup"><span data-stu-id="c1d65-476">Manually enter a location.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="c1d65-477"><strong>Overstyr plassering</strong>-handlingen er ikke mulig.</span><span class="sxs-lookup"><span data-stu-id="c1d65-477">The <strong>Override location</strong> action isn't possible.</span></span> <span data-ttu-id="c1d65-478">Den mislykkes, og det oppstår en feil.</span><span class="sxs-lookup"><span data-stu-id="c1d65-478">It fails, and an error is thrown.</span></span></td>
<td><span data-ttu-id="c1d65-479">Gjelder ikke</span><span class="sxs-lookup"><span data-stu-id="c1d65-479">Not applicable</span></span></td>
</tr>
</tbody>
</table>

#### <a name="full-button--split-a-work-line-because-of-overflow-on-the-user-location"></a><span data-ttu-id="c1d65-480">Full knapp – del opp en arbeidslinje på grunn av overflyt på brukerplasseringen</span><span class="sxs-lookup"><span data-stu-id="c1d65-480">Full button – Split a work line because of overflow on the user location</span></span>

<table>
<thead>
<tr>
<th><span data-ttu-id="c1d65-481">Nøkkeloppsettsparameter</span><span class="sxs-lookup"><span data-stu-id="c1d65-481">Key setup parameter</span></span></th>
<th><span data-ttu-id="c1d65-482">Partikvantitet er tilgjengelig</span><span class="sxs-lookup"><span data-stu-id="c1d65-482">Batch quantity is available</span></span></th>
<th><span data-ttu-id="c1d65-483">Nøkkelbrukertrinn</span><span class="sxs-lookup"><span data-stu-id="c1d65-483">Key user steps</span></span></th>
<th><span data-ttu-id="c1d65-484">Lagerarbeid</span><span class="sxs-lookup"><span data-stu-id="c1d65-484">Warehouse work</span></span></th>
<th><span data-ttu-id="c1d65-485">Ordreigangsatt partireservasjon</span><span class="sxs-lookup"><span data-stu-id="c1d65-485">Order-committed batch reservation</span></span></th>
</tr>
</thead>
<tbody>
<tr>
<td><span data-ttu-id="c1d65-486"><strong>Tillat oppdeling av arbeid</strong>-alternativet er aktivert på mobilenhetens menyelement.</span><span class="sxs-lookup"><span data-stu-id="c1d65-486">The <strong>Allow splitting of work</strong> option is enabled on the mobile device menu item.</span></span></td>
<td><span data-ttu-id="c1d65-487">Gjelder ikke</span><span class="sxs-lookup"><span data-stu-id="c1d65-487">Not applicable</span></span></td>
<td>
<ol>
<li><span data-ttu-id="c1d65-488">Velg menyelementet <strong>Full</strong> på lagerappen når du behandler plukkarbeid.</span><span class="sxs-lookup"><span data-stu-id="c1d65-488">Select the <strong>Full</strong> menu item on the warehouse app when you process picking work.</span></span></li>
<li><span data-ttu-id="c1d65-489">I <strong>Plukkvantitet</strong>-feltet angir du en delvis kvantitet for den påkrevde plukkingen for å indikere hele kapasiteten.</span><span class="sxs-lookup"><span data-stu-id="c1d65-489">In the <strong>Pick Qty</strong> field, enter a partial quantity of the required pick to indicate the full capacity.</span></span></li>
</ol>
</td>
<td>
<ul>
<li><span data-ttu-id="c1d65-490">I gjeldende arbeid oppdateres kvantiteten til den gjenværende kvantiteten som må plukkes.</span><span class="sxs-lookup"><span data-stu-id="c1d65-490">On the current work, the quantity is updated to the remaining quantity that must be picked.</span></span></li>
<li><span data-ttu-id="c1d65-491">Nytt arbeid for den plukkede kvantiteten opprettes og lukkes.</span><span class="sxs-lookup"><span data-stu-id="c1d65-491">New work for the picked quantity is created and closed.</span></span></li>
</ul></td>
<td><span data-ttu-id="c1d65-492">Gjelder ikke</span><span class="sxs-lookup"><span data-stu-id="c1d65-492">Not applicable</span></span></td>
</tr>
</tbody>
</table>

#### <a name="reduce-the-picked-quantity-of-completed-work-from-a-load"></a><span data-ttu-id="c1d65-493">Redusere den plukkede kvantiteten for fullført arbeid (fra en belastning)</span><span class="sxs-lookup"><span data-stu-id="c1d65-493">Reduce the picked quantity of completed work (from a load)</span></span>

<table>
<thead>
<tr>
<th><span data-ttu-id="c1d65-494">Nøkkeloppsettsparameter</span><span class="sxs-lookup"><span data-stu-id="c1d65-494">Key setup parameter</span></span></th>
<th><span data-ttu-id="c1d65-495">Partikvantitet er tilgjengelig</span><span class="sxs-lookup"><span data-stu-id="c1d65-495">Batch quantity is available</span></span></th>
<th><span data-ttu-id="c1d65-496">Nøkkelbrukertrinn</span><span class="sxs-lookup"><span data-stu-id="c1d65-496">Key user steps</span></span></th>
<th><span data-ttu-id="c1d65-497">Lagerarbeid</span><span class="sxs-lookup"><span data-stu-id="c1d65-497">Warehouse work</span></span></th>
<th><span data-ttu-id="c1d65-498">Ordreigangsatt partireservasjon</span><span class="sxs-lookup"><span data-stu-id="c1d65-498">Order-committed batch reservation</span></span></th>
</tr>
</thead>
<tbody>
<tr>
<td rowspan='2'><span data-ttu-id="c1d65-499">Gjelder ikke</span><span class="sxs-lookup"><span data-stu-id="c1d65-499">Not applicable</span></span></td>
<td><span data-ttu-id="c1d65-500">Ja</span><span class="sxs-lookup"><span data-stu-id="c1d65-500">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="c1d65-501">Åpne <strong>Reduser plukket kvantitet</strong>-siden fra belastningslinjen.</span><span class="sxs-lookup"><span data-stu-id="c1d65-501">Open the <strong>Reduce picked quantity</strong> page from the load line.</span></span></li>
<li><span data-ttu-id="c1d65-502">Angi den fullstendige kvantiteten som skal plukkes vekk.</span><span class="sxs-lookup"><span data-stu-id="c1d65-502">Enter the full quantity to unpick.</span></span></li>
<li><span data-ttu-id="c1d65-503">Velg "Flytt til"-plassering/-nummerskilt.</span><span class="sxs-lookup"><span data-stu-id="c1d65-503">Select a "move to" location/license plate.</span></span></li>
</ol>
</td>
<td>
<ul> 
<li><span data-ttu-id="c1d65-504">Arbeid som er knyttet til belastningslinjen, avbrytes.</span><span class="sxs-lookup"><span data-stu-id="c1d65-504">Work that is associated with the load line is canceled.</span></span></li>
<li><span data-ttu-id="c1d65-505">Nytt arbeid for beholdningsbevegelsen opprettes og lukkes.</span><span class="sxs-lookup"><span data-stu-id="c1d65-505">New work for the inventory movement is created and closed.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="c1d65-506">Kvantiteten reserveres på nytt for samme parti.</span><span class="sxs-lookup"><span data-stu-id="c1d65-506">The quantity is re-reserved for the same batch.</span></span> <span data-ttu-id="c1d65-507">Systemet tilordner tilfeldig en plassering og et nummerskilt (hvis plasseringen er nummerskiltstyrt) der kvantiteten er tilgjengelig.</span><span class="sxs-lookup"><span data-stu-id="c1d65-507">The system randomly assigns a location and license plate (if the location is license plate–controlled) where the quantity is available.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="c1d65-508">Nei</span><span class="sxs-lookup"><span data-stu-id="c1d65-508">No</span></span></td>
<td><span data-ttu-id="c1d65-509">Se den forrige raden.</span><span class="sxs-lookup"><span data-stu-id="c1d65-509">See the previous row.</span></span></td>
<td><span data-ttu-id="c1d65-510">Se den forrige raden.</span><span class="sxs-lookup"><span data-stu-id="c1d65-510">See the previous row.</span></span></td>
<td><span data-ttu-id="c1d65-511">Kvantiteten reserveres på nytt for samme parti, samt for samme plassering og nummerskilt (hvis plasseringen er nummerskiltstyrt) som ble angitt under vekkplukking.</span><span class="sxs-lookup"><span data-stu-id="c1d65-511">The quantity is re-reserved for the same batch, and for the same location and license plate (if the location is license plate–controlled) that were entered during unpicking.</span></span></td>
</tr>
</tbody>
</table>

#### <a name="move-an-item-within-a-warehouse"></a><span data-ttu-id="c1d65-512">Flytte en vare i et lager</span><span class="sxs-lookup"><span data-stu-id="c1d65-512">Move an item within a warehouse</span></span>

> [!NOTE]
> <span data-ttu-id="c1d65-513">Denne lagerhandlingen gjelder bare for bevegelse av **Arbeidsopprettelsesprosess**-typen, ikke for bevegelse etter mal.</span><span class="sxs-lookup"><span data-stu-id="c1d65-513">This warehouse action is applicable only to movement of the **Work creation process** type, not to movement by template.</span></span>

<table>
<thead>
<tr>
<th><span data-ttu-id="c1d65-514">Nøkkeloppsettsparameter</span><span class="sxs-lookup"><span data-stu-id="c1d65-514">Key setup parameter</span></span></th>
<th><span data-ttu-id="c1d65-515">Partikvantitet er tilgjengelig</span><span class="sxs-lookup"><span data-stu-id="c1d65-515">Batch quantity is available</span></span></th>
<th><span data-ttu-id="c1d65-516">Nøkkelbrukertrinn</span><span class="sxs-lookup"><span data-stu-id="c1d65-516">Key user steps</span></span></th>
<th><span data-ttu-id="c1d65-517">Lagerarbeid</span><span class="sxs-lookup"><span data-stu-id="c1d65-517">Warehouse work</span></span></th>
<th><span data-ttu-id="c1d65-518">Ordreigangsatt partireservasjon</span><span class="sxs-lookup"><span data-stu-id="c1d65-518">Order-committed batch reservation</span></span></th>
</tr>
</thead>
<tbody>
<tr>
<td rowspan='2'><span data-ttu-id="c1d65-519"><strong>Tillat bevegelse av beholdning med tilhørende arbeid</strong>-alternativet er aktivert for arbeideren.</span><span class="sxs-lookup"><span data-stu-id="c1d65-519">The <strong>Allow movement of inventory with associated work</strong> option is enabled on the worker.</span></span></td>
<td><span data-ttu-id="c1d65-520">Ja</span><span class="sxs-lookup"><span data-stu-id="c1d65-520">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="c1d65-521">Start en flytting på lagerappen.</span><span class="sxs-lookup"><span data-stu-id="c1d65-521">Start a movement on the warehouse app.</span></span></li>
<li><span data-ttu-id="c1d65-522">Angi "fra"- og "til"-plasseringene.</span><span class="sxs-lookup"><span data-stu-id="c1d65-522">Enter "from" and "to" locations.</span></span></li>
</ol></td>
<td>
<ul>
<li><span data-ttu-id="c1d65-523">For alt eksisterende arbeid som påvirkes av flyttingen, oppdateres plukkplasseringen til den nye plasseringen.</span><span class="sxs-lookup"><span data-stu-id="c1d65-523">On all existing work that is affected by the move, the pick location is updated to the new "to" location.</span></span></li>
<li><span data-ttu-id="c1d65-524">Nytt arbeid for beholdningsbevegelsen opprettes og lukkes.</span><span class="sxs-lookup"><span data-stu-id="c1d65-524">New work for the inventory movement is created and closed.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="c1d65-525">Alle eksisterende reservasjoner som påvirkes av kvantitetsbevegelsen fra den gitte plasseringen, reserveres på nytt for samme parti.</span><span class="sxs-lookup"><span data-stu-id="c1d65-525">All existing reservations that are affected by the quantity movement from the given location are re-reserved for the same batch.</span></span> <span data-ttu-id="c1d65-526">Systemet tilordner tilfeldig en plassering og et nummerskilt (hvis plasseringen er nummerskiltstyrt) der kvantiteten er tilgjengelig.</span><span class="sxs-lookup"><span data-stu-id="c1d65-526">The system randomly assigns a location and license plate (if the location is license plate–controlled) where the quantity is available.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="c1d65-527">Nei</span><span class="sxs-lookup"><span data-stu-id="c1d65-527">No</span></span></td>
<td><span data-ttu-id="c1d65-528">Se den forrige raden.</span><span class="sxs-lookup"><span data-stu-id="c1d65-528">See the previous row.</span></span></td>
<td><span data-ttu-id="c1d65-529">Se den forrige raden.</span><span class="sxs-lookup"><span data-stu-id="c1d65-529">See the previous row.</span></span></td>
<td><span data-ttu-id="c1d65-530">Alle eksisterende reservasjoner som påvirkes av kvantitetsbevegelsen fra den gitte plasseringen, reserveres for samme parti, samt for den nye "til"- plasseringen og det nye "til"-nummerskiltet (hvis plasseringen er nummerskiltstyrt).</span><span class="sxs-lookup"><span data-stu-id="c1d65-530">All existing reservations that are affected by the quantity movement from the given location are re-reserved for the same batch, and for the new "to" location and license plate (if the location is license plate–controlled).</span></span></td>
</tr>
</tbody>
</table>

#### <a name="reverse-the-picked-quantity-of-completed-work-from-a-load-or-a-wave"></a><span data-ttu-id="c1d65-531">Reversere den plukkede kvantiteten for fullført arbeid (fra en belastning eller en bølge)</span><span class="sxs-lookup"><span data-stu-id="c1d65-531">Reverse the picked quantity of completed work (from a load or a wave)</span></span>

<table>
<thead>
<tr>
<th><span data-ttu-id="c1d65-532">Nøkkeloppsettsparameter</span><span class="sxs-lookup"><span data-stu-id="c1d65-532">Key setup parameter</span></span></th>
<th><span data-ttu-id="c1d65-533">Partikvantitet er tilgjengelig</span><span class="sxs-lookup"><span data-stu-id="c1d65-533">Batch quantity is available</span></span></th>
<th><span data-ttu-id="c1d65-534">Nøkkelbrukertrinn</span><span class="sxs-lookup"><span data-stu-id="c1d65-534">Key user steps</span></span></th>
<th><span data-ttu-id="c1d65-535">Lagerarbeid</span><span class="sxs-lookup"><span data-stu-id="c1d65-535">Warehouse work</span></span></th>
<th><span data-ttu-id="c1d65-536">Ordreigangsatt partireservasjon</span><span class="sxs-lookup"><span data-stu-id="c1d65-536">Order-committed batch reservation</span></span></th>
</tr>
</thead>
<tbody>
<tr>
<td rowspan='6'><span data-ttu-id="c1d65-537">Gjelder ikke</span><span class="sxs-lookup"><span data-stu-id="c1d65-537">Not applicable</span></span></td>
<td><span data-ttu-id="c1d65-538">Ja</span><span class="sxs-lookup"><span data-stu-id="c1d65-538">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="c1d65-539">Åpne <strong>Reverser arbeid</strong>-siden.</span><span class="sxs-lookup"><span data-stu-id="c1d65-539">Open the <strong>Reverse work</strong> page.</span></span></li>
<li><span data-ttu-id="c1d65-540">På forespørselssiden velger du <strong>Etterlat varer på gjeldende plassering</strong>-alternativet.</span><span class="sxs-lookup"><span data-stu-id="c1d65-540">On the request page, select the <strong>Leave items at current location</strong> option.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="c1d65-541">Alt arbeid som er knyttet til belastningen, avbrytes.</span><span class="sxs-lookup"><span data-stu-id="c1d65-541">All work that is associated with the load is canceled.</span></span></td>
<td><span data-ttu-id="c1d65-542">Kvantiteten reserveres på nytt for samme parti.</span><span class="sxs-lookup"><span data-stu-id="c1d65-542">The quantity is re-reserved for the same batch.</span></span> <span data-ttu-id="c1d65-543">Systemet tilordner tilfeldig en plassering og et nummerskilt (hvis plasseringen er nummerskiltstyrt) der kvantiteten er tilgjengelig.</span><span class="sxs-lookup"><span data-stu-id="c1d65-543">The system randomly assigns a location and license plate (if the location is license plate–controlled) where the quantity is available.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="c1d65-544">Nei</span><span class="sxs-lookup"><span data-stu-id="c1d65-544">No</span></span></td>
<td><span data-ttu-id="c1d65-545">Se den forrige raden.</span><span class="sxs-lookup"><span data-stu-id="c1d65-545">See the previous row.</span></span></td>
<td><span data-ttu-id="c1d65-546">Se den forrige raden.</span><span class="sxs-lookup"><span data-stu-id="c1d65-546">See the previous row.</span></span></td>
<td><span data-ttu-id="c1d65-547">Kvantiteten reserveres på nytt for samme parti, samt for plasseringen og nummerskiltet der kvantiteten ble etterlatt ved reversering.</span><span class="sxs-lookup"><span data-stu-id="c1d65-547">The quantity is re-reserved for the same batch, and for the location and license plate where the quantity was left upon reversal.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="c1d65-548">Ja</span><span class="sxs-lookup"><span data-stu-id="c1d65-548">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="c1d65-549">Åpne <strong>Reverser arbeid</strong>-siden.</span><span class="sxs-lookup"><span data-stu-id="c1d65-549">Open the <strong>Reverse work</strong> page.</span></span></li>
<li><span data-ttu-id="c1d65-550">På forespørselssiden velger du <strong>Tilordne varer til denne plasseringen</strong>-alternativet.</span><span class="sxs-lookup"><span data-stu-id="c1d65-550">On the request page, select the <strong>Assign items to this location</strong> option.</span></span></li>
</ol>
</td>
<td>
<ul>
<li><span data-ttu-id="c1d65-551">Det gjeldende arbeidet avbrytes.</span><span class="sxs-lookup"><span data-stu-id="c1d65-551">The current work is canceled.</span></span></li>
<li><span data-ttu-id="c1d65-552">Nytt arbeid for beholdningsbevegelsen opprettes og lukkes.</span><span class="sxs-lookup"><span data-stu-id="c1d65-552">New work for the inventory movement is created and closed.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="c1d65-553">Kvantiteten reserveres på nytt for samme parti.</span><span class="sxs-lookup"><span data-stu-id="c1d65-553">The quantity is re-reserved for the same batch.</span></span> <span data-ttu-id="c1d65-554">Systemet tilordner tilfeldig en plassering og et nummerskilt (hvis plasseringen er nummerskiltstyrt) der kvantiteten er tilgjengelig.</span><span class="sxs-lookup"><span data-stu-id="c1d65-554">The system randomly assigns a location and license plate (if the location is license plate–controlled) where the quantity is available.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="c1d65-555">Nei</span><span class="sxs-lookup"><span data-stu-id="c1d65-555">No</span></span></td>
<td><span data-ttu-id="c1d65-556">Se den forrige raden.</span><span class="sxs-lookup"><span data-stu-id="c1d65-556">See the previous row.</span></span></td>
<td><span data-ttu-id="c1d65-557">Se den forrige raden.</span><span class="sxs-lookup"><span data-stu-id="c1d65-557">See the previous row.</span></span></td>
<td><span data-ttu-id="c1d65-558">Kvantiteten reserveres på nytt for samme parti, samt for plasseringen og nummerskiltet som kvantiteten ble tilordnet til ved reversering.</span><span class="sxs-lookup"><span data-stu-id="c1d65-558">The quantity is re-reserved for the same batch, and for the location and license plate that the quantity was assigned to upon reversal.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="c1d65-559">Ja/nei</span><span class="sxs-lookup"><span data-stu-id="c1d65-559">Yes/No</span></span></td>
<td>
<ol>
<li><span data-ttu-id="c1d65-560">Åpne <strong>Reverser arbeid</strong>-siden.</span><span class="sxs-lookup"><span data-stu-id="c1d65-560">Open the <strong>Reverse work</strong> page.</span></span></li>
<li><span data-ttu-id="c1d65-561">På forespørselssiden velger du <strong>Flytt varer til denne plasseringen</strong>-alternativet.</span><span class="sxs-lookup"><span data-stu-id="c1d65-561">On the request page, select the <strong>Move items to this location</strong> option.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="c1d65-562">Reversering støttes ikke.</span><span class="sxs-lookup"><span data-stu-id="c1d65-562">Reversal isn't supported.</span></span></td>
<td><span data-ttu-id="c1d65-563">Gjelder ikke</span><span class="sxs-lookup"><span data-stu-id="c1d65-563">Not applicable</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="c1d65-564">Ja/nei</span><span class="sxs-lookup"><span data-stu-id="c1d65-564">Yes/No</span></span></td>
<td>
<ol>
<li><span data-ttu-id="c1d65-565">Åpne <strong>Reverser arbeid</strong>-siden.</span><span class="sxs-lookup"><span data-stu-id="c1d65-565">Open the <strong>Reverse work</strong> page.</span></span></li>
<li><span data-ttu-id="c1d65-566">På forespørselssiden velger du <strong>Flytt varer basert på plasseringsdirektiver</strong>-alternativet.</span><span class="sxs-lookup"><span data-stu-id="c1d65-566">On the request page, select the <strong>Move items based on location directives</strong> option.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="c1d65-567">Reversering støttes ikke.</span><span class="sxs-lookup"><span data-stu-id="c1d65-567">Reversal isn't supported.</span></span> </td>
<td><span data-ttu-id="c1d65-568">Gjelder ikke</span><span class="sxs-lookup"><span data-stu-id="c1d65-568">Not applicable</span></span></td>
</tr>
</tbody>
</table>

#### <a name="short-pick-a-quantity--register-the-quantity-as-physically-not-found-at-the-locationlicense-plate-while-you-perform-picking-work"></a><span data-ttu-id="c1d65-569">Plukk en kvantitet med mangler – Registrer kvantiteten som fysisk ikke funnet på plasseringen/nummerskiltet mens du utfører plukkarbeidet</span><span class="sxs-lookup"><span data-stu-id="c1d65-569">Short-pick a quantity – Register the quantity as physically not found at the location/license plate while you perform picking work</span></span>

<table>
<thead>
<tr>
<th><span data-ttu-id="c1d65-570">Nøkkeloppsettsparameter</span><span class="sxs-lookup"><span data-stu-id="c1d65-570">Key setup parameter</span></span></th>
<th><span data-ttu-id="c1d65-571">Partikvantitet er tilgjengelig</span><span class="sxs-lookup"><span data-stu-id="c1d65-571">Batch quantity is available</span></span></th>
<th><span data-ttu-id="c1d65-572">Nøkkelbrukertrinn</span><span class="sxs-lookup"><span data-stu-id="c1d65-572">Key user steps</span></span></th>
<th><span data-ttu-id="c1d65-573">Lagerarbeid</span><span class="sxs-lookup"><span data-stu-id="c1d65-573">Warehouse work</span></span></th>
<th><span data-ttu-id="c1d65-574">Ordreigangsatt partireservasjon</span><span class="sxs-lookup"><span data-stu-id="c1d65-574">Order-committed batch reservation</span></span></th>
</tr>
</thead>
<tbody>
<tr>
<td rowspan='2'><span data-ttu-id="c1d65-575">Et arbeidsunntak av <strong>Plukk med mangler</strong>-typen settes opp, der <strong>Ny fordeling av vare</strong> = <strong>Ingen</strong>, <strong>Juster beholdning</strong> = <strong>Ja</strong> og <strong>Fjern reservasjoner</strong> = <strong>Nei</strong>.</span><span class="sxs-lookup"><span data-stu-id="c1d65-575">A work exception of the <strong>Short pick</strong> type is set up, where <strong>Item reallocation</strong> = <strong>None</strong>, <strong>Adjust inventory</strong> = <strong>Yes</strong>, and <strong>Remove reservations</strong> = <strong>No</strong>.</span></span></td>
<td><span data-ttu-id="c1d65-576">Ja</span><span class="sxs-lookup"><span data-stu-id="c1d65-576">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="c1d65-577">Velg <strong>Plukk med mangler</strong>-menyelementet på lagerappen når du kjører plukkarbeid.</span><span class="sxs-lookup"><span data-stu-id="c1d65-577">Select the <strong>Shortpick</strong> menu item on the warehouse app when you run picking work.</span></span></li>
<li><span data-ttu-id="c1d65-578">I feltet <strong>Plukkekvantitet</strong> angir du <strong>0</strong> (null).</span><span class="sxs-lookup"><span data-stu-id="c1d65-578">In the <strong>Pick Quantity</strong> field, enter <strong>0</strong> (zero).</span></span></li>
<li><span data-ttu-id="c1d65-579">I <strong>Årsak</strong>-feltet angir du <strong>Ingen ny fordeling</strong>.</span><span class="sxs-lookup"><span data-stu-id="c1d65-579">In the <strong>Reason</strong> field, enter <strong>No reallocation</strong>.</span></span></li>
</ol>
</td>
<td>
<ul>
<li><span data-ttu-id="c1d65-580">Gjeldende arbeid lukkes, og den plukkede kvantiteten er 0 (null).</span><span class="sxs-lookup"><span data-stu-id="c1d65-580">The current work is closed, and the picked quantity is 0 (zero).</span></span></li>
<li><span data-ttu-id="c1d65-581">Det opprettes en beholdningstransaksjon av <strong>Opptelling</strong>-typen, og <strong>Solgt</strong>-utstedelsestypen opprettes for å representere justeringen.</span><span class="sxs-lookup"><span data-stu-id="c1d65-581">An inventory transaction of the <strong>Counting</strong> type and the <strong>Sold</strong> issue type is created to represent the adjustment.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="c1d65-582">Kvantiteten reserveres på nytt for samme parti.</span><span class="sxs-lookup"><span data-stu-id="c1d65-582">The quantity is re-reserved for the same batch.</span></span> <span data-ttu-id="c1d65-583">Systemet tilordner tilfeldig en plassering og et nummerskilt (hvis plasseringen er nummerskiltstyrt) der kvantiteten er tilgjengelig.</span><span class="sxs-lookup"><span data-stu-id="c1d65-583">The system randomly assigns a location and license plate (if the location is license plate–controlled) where the quantity is available.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="c1d65-584">Nei</span><span class="sxs-lookup"><span data-stu-id="c1d65-584">No</span></span></td>
<td><span data-ttu-id="c1d65-585">Se den forrige raden.</span><span class="sxs-lookup"><span data-stu-id="c1d65-585">See the previous row.</span></span></td>
<td>
<ul>
<li><span data-ttu-id="c1d65-586">Handlingen for plukking med mangler mislykkes, og det oppstår en feil.</span><span class="sxs-lookup"><span data-stu-id="c1d65-586">The short-picking action fails, and an error is thrown.</span></span></li>
<li><span data-ttu-id="c1d65-587">Gjeldende arbeid forblir åpent.</span><span class="sxs-lookup"><span data-stu-id="c1d65-587">The current work remains open.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="c1d65-588">Gjelder ikke</span><span class="sxs-lookup"><span data-stu-id="c1d65-588">Not applicable</span></span></td>
</tr>
<tr>
<td rowspan='2'><span data-ttu-id="c1d65-589">Et arbeidsunntak av <strong>Plukk med mangler</strong>-typen settes opp, der <strong>Ny fordeling av vare</strong> = <strong>Ingen</strong>, <strong>Juster beholdning</strong> = <strong>Ja</strong> og <strong>Fjern reservasjoner</strong> = <strong>Ja</strong>.</span><span class="sxs-lookup"><span data-stu-id="c1d65-589">A work exception of the <strong>Short pick</strong> type is set up, where <strong>Item reallocation</strong> = <strong>None</strong>, <strong>Adjust inventory</strong> = <strong>Yes</strong>, and <strong>Remove reservations</strong> = <strong>Yes</strong>.</span></span></td>
<td><span data-ttu-id="c1d65-590">Ja</span><span class="sxs-lookup"><span data-stu-id="c1d65-590">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="c1d65-591">Velg <strong>Plukk med mangler</strong>-menyelementet på lagerappen når du kjører plukkarbeid.</span><span class="sxs-lookup"><span data-stu-id="c1d65-591">Select the <strong>Shortpick</strong> menu item on the warehouse app when you run picking work.</span></span></li>
<li><span data-ttu-id="c1d65-592">I feltet <strong>Plukkekvantitet</strong> angir du <strong>0</strong> (null).</span><span class="sxs-lookup"><span data-stu-id="c1d65-592">In the <strong>Pick Quantity</strong> field, enter <strong>0</strong> (zero).</span></span></li>
<li><span data-ttu-id="c1d65-593">I <strong>Årsak</strong>-feltet angir du <strong>Ingen ny fordeling</strong>.</span><span class="sxs-lookup"><span data-stu-id="c1d65-593">In the <strong>Reason</strong> field, enter <strong>No reallocation</strong>.</span></span></li>
</ol>
</td>
<td>
<ul>
<li><span data-ttu-id="c1d65-594">Gjeldende arbeid lukkes, og den plukkede kvantiteten er 0 (null).</span><span class="sxs-lookup"><span data-stu-id="c1d65-594">The current work is closed, and the picked quantity is 0 (zero).</span></span></li>
<li><span data-ttu-id="c1d65-595">Det opprettes en beholdningstransaksjon av <strong>Opptelling</strong>-typen, og <strong>Solgt</strong>-utstedelsestypen opprettes for å representere justeringen.</span><span class="sxs-lookup"><span data-stu-id="c1d65-595">An inventory transaction of the <strong>Counting</strong> type and the <strong>Sold</strong> issue type is created to represent the adjustment.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="c1d65-596">Alle eksisterende reservasjoner som påvirkes av kvantitetsjusteringen i plasseringen for plukking med mangler, reserveres på nytt for samme parti.</span><span class="sxs-lookup"><span data-stu-id="c1d65-596">All existing reservations that are affected by the quantity adjustment in the short-picked location are re-reserved for the same batch.</span></span> <span data-ttu-id="c1d65-597">Systemet tilordner tilfeldig en plassering og et nummerskilt (hvis plasseringen er nummerskiltstyrt) der kvantiteten er tilgjengelig.</span><span class="sxs-lookup"><span data-stu-id="c1d65-597">The system randomly assigns a location and license plate (if the location is license plate–controlled) where the quantity is available.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="c1d65-598">Nei</span><span class="sxs-lookup"><span data-stu-id="c1d65-598">No</span></span></td>
<td><span data-ttu-id="c1d65-599">Se den forrige raden.</span><span class="sxs-lookup"><span data-stu-id="c1d65-599">See the previous row.</span></span></td>
<td><span data-ttu-id="c1d65-600">Se den forrige raden.</span><span class="sxs-lookup"><span data-stu-id="c1d65-600">See the previous row.</span></span></td>
<td><span data-ttu-id="c1d65-601">Alle eksisterende reservasjoner som påvirkes av kvantitetsjusteringen i plasseringen for plukking med mangler, fjernes.</span><span class="sxs-lookup"><span data-stu-id="c1d65-601">All existing reservations that are affected by the quantity adjustment in the short-picked location are removed.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="c1d65-602">Et arbeidsunntak av <strong>Plukk med mangler</strong>-typen settes opp, der <strong>Ny fordeling av vare</strong> = <strong>Manuell</strong>, <strong>Juster beholdning</strong> = <strong>Ja</strong> og <strong>Fjern reservasjoner</strong> = <strong>Nei/Ja</strong>.</span><span class="sxs-lookup"><span data-stu-id="c1d65-602">A work exception of the <strong>Short pick</strong> type is set up, where <strong>Item reallocation</strong> = <strong>Manual</strong>, <strong>Adjust inventory</strong> = <strong>Yes</strong>, and <strong>Remove reservations</strong> = <strong>No/Yes</strong>.</span></span> <span data-ttu-id="c1d65-603">I tillegg er <strong>Tillat manuell omfordeling av varer</strong>-alternativet aktivert for arbeideren.</span><span class="sxs-lookup"><span data-stu-id="c1d65-603">Additionally, the <strong>Allow manual item reallocation</strong> option is enabled on the worker.</span></span></td>
<td><span data-ttu-id="c1d65-604">Ja</span><span class="sxs-lookup"><span data-stu-id="c1d65-604">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="c1d65-605">Velg <strong>Plukk med mangler</strong>-menyelementet på lagerappen når du kjører plukkarbeid.</span><span class="sxs-lookup"><span data-stu-id="c1d65-605">Select the <strong>Shortpick</strong> menu item on the warehouse app when you run picking work.</span></span></li>
<li><span data-ttu-id="c1d65-606">I <strong>Kvantitet for plukking med mangler</strong>-feltet angir du <strong>0</strong> (null).</span><span class="sxs-lookup"><span data-stu-id="c1d65-606">In the <strong>Shortpick Quantity</strong> field, enter <strong>0</strong> (zero).</span></span></li>
<li><span data-ttu-id="c1d65-607">I <strong>Årsak</strong>-feltet velger du <strong>Plukking med mangler med manuell omfordeling</strong>.</span><span class="sxs-lookup"><span data-stu-id="c1d65-607">In the <strong>Reason</strong> field, select <strong>Short Picking with manual reallocation</strong>.</span></span></li>
<li><span data-ttu-id="c1d65-608">Velg plasseringen/nummerskiltet i listen.</span><span class="sxs-lookup"><span data-stu-id="c1d65-608">Select the location/license plate in the list.</span></span></li>
</ol>
</td>
<td>
<ul>
<li><span data-ttu-id="c1d65-609">For gjeldende arbeid oppstår det følgende handlinger:</span><span class="sxs-lookup"><span data-stu-id="c1d65-609">On the current work, the following actions occur:</span></span>
<ul>
<li><span data-ttu-id="c1d65-610">Plukklinjen lukkes, og den plukkede kvantiteten er 0 (null).</span><span class="sxs-lookup"><span data-stu-id="c1d65-610">The pick line is closed, and the picked quantity is 0 (zero).</span></span></li>
<li><span data-ttu-id="c1d65-611">Plasseringslinjen avbrytes.</span><span class="sxs-lookup"><span data-stu-id="c1d65-611">The put line is canceled.</span></span></li>
<li><span data-ttu-id="c1d65-612">Det opprettes en ny plukklinje.</span><span class="sxs-lookup"><span data-stu-id="c1d65-612">A new pick line is created.</span></span> <span data-ttu-id="c1d65-613">Den bruker plasseringen/nummerskiltet som brukeren valgte.</span><span class="sxs-lookup"><span data-stu-id="c1d65-613">It uses the location/license plate that the user selected.</span></span></li>
<li><span data-ttu-id="c1d65-614">Det opprettes en ny plasseringslinje.</span><span class="sxs-lookup"><span data-stu-id="c1d65-614">A new put line is created.</span></span></li>
</ul>
</li>
<li><span data-ttu-id="c1d65-615">Det opprettes en beholdningstransaksjon av <strong>Opptelling</strong>-typen, og <strong>Solgt</strong>-utstedelsestypen opprettes for å representere justeringen.</span><span class="sxs-lookup"><span data-stu-id="c1d65-615">An inventory transaction of the <strong>Counting</strong> type and the <strong>Sold</strong> issue type is created to represent the adjustment.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="c1d65-616">Gjelder ikke</span><span class="sxs-lookup"><span data-stu-id="c1d65-616">Not applicable</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="c1d65-617">Et arbeidsunntak av <strong>Plukk med mangler</strong>-typen settes opp, der <strong>Ny fordeling av vare</strong> = <strong>Manuell</strong>, <strong>Juster beholdning</strong> = <strong>Ja</strong> og <strong>Fjern reservasjoner</strong> = <strong>Nei</strong>.</span><span class="sxs-lookup"><span data-stu-id="c1d65-617">A work exception of the <strong>Short pick</strong> type is set up, where <strong>Item reallocation</strong> = <strong>Manual</strong>, <strong>Adjust inventory</strong> = <strong>Yes</strong>, and <strong>Remove reservations</strong> = <strong>No</strong>.</span></span> <span data-ttu-id="c1d65-618">I tillegg er <strong>Tillat manuell omfordeling av varer</strong>-alternativet aktivert for arbeideren.</span><span class="sxs-lookup"><span data-stu-id="c1d65-618">Additionally, the <strong>Allow manual item reallocation</strong> option is enabled on the worker.</span></span></td>
<td><span data-ttu-id="c1d65-619">Nei</span><span class="sxs-lookup"><span data-stu-id="c1d65-619">No</span></span></td>
<td>
<ol>
<li><span data-ttu-id="c1d65-620">Velg <strong>Plukk med mangler</strong>-menyelementet på lagerappen når du kjører plukkarbeid.</span><span class="sxs-lookup"><span data-stu-id="c1d65-620">Select the <strong>Shortpick</strong> menu item on the warehouse app when you run picking work.</span></span></li>
<li><span data-ttu-id="c1d65-621">I <strong>Kvantitet for plukking med mangler</strong>-feltet angir du <strong>0</strong> (null).</span><span class="sxs-lookup"><span data-stu-id="c1d65-621">In the <strong>Shortpick Quantity</strong> field, enter <strong>0</strong> (zero).</span></span></li>
<li><span data-ttu-id="c1d65-622">I <strong>Årsak</strong>-feltet velger du <strong>Plukking med mangler med manuell omfordeling</strong>.</span><span class="sxs-lookup"><span data-stu-id="c1d65-622">In the <strong>Reason</strong> field, select <strong>Short Picking with manual reallocation</strong>.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="c1d65-623">Handlingen for plukking med mangler mislykkes, og det oppstår en feil.</span><span class="sxs-lookup"><span data-stu-id="c1d65-623">The short-picking action fails, and an error is thrown.</span></span></td>
<td><span data-ttu-id="c1d65-624">Gjelder ikke</span><span class="sxs-lookup"><span data-stu-id="c1d65-624">Not applicable</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="c1d65-625">Et arbeidsunntak av <strong>Plukk med mangler</strong>-typen settes opp, der <strong>Ny fordeling av vare</strong> = <strong>Manuell</strong>, <strong>Juster beholdning</strong> = <strong>Ja</strong> og <strong>Fjern reservasjoner</strong> = <strong>Ja</strong>.</span><span class="sxs-lookup"><span data-stu-id="c1d65-625">A work exception of the <strong>Short pick</strong> type is set up, where <strong>Item reallocation</strong> = <strong>Manual</strong>, <strong>Adjust inventory</strong> = <strong>Yes</strong>, and <strong>Remove reservations</strong> = <strong>Yes</strong>.</span></span> <span data-ttu-id="c1d65-626">I tillegg er <strong>Tillat manuell omfordeling av varer</strong>-alternativet aktivert for arbeideren.</span><span class="sxs-lookup"><span data-stu-id="c1d65-626">Additionally, the <strong>Allow manual item reallocation</strong> option is enabled on the worker.</span></span></td>
<td><span data-ttu-id="c1d65-627">Nei</span><span class="sxs-lookup"><span data-stu-id="c1d65-627">No</span></span></td>
<td>
<ol>
<li><span data-ttu-id="c1d65-628">Velg <strong>Plukk med mangler</strong>-menyelementet på lagerappen når du kjører plukkarbeid.</span><span class="sxs-lookup"><span data-stu-id="c1d65-628">Select the <strong>Shortpick</strong> menu item on the warehouse app when you run picking work.</span></span></li>
<li><span data-ttu-id="c1d65-629">I <strong>Kvantitet for plukking med mangler</strong>-feltet angir du <strong>0</strong> (null).</span><span class="sxs-lookup"><span data-stu-id="c1d65-629">In the <strong>Shortpick Quantity</strong> field, enter <strong>0</strong> (zero).</span></span></li>
<li><span data-ttu-id="c1d65-630">I <strong>Årsak</strong>-feltet velger du <strong>Plukking med mangler med manuell omfordeling</strong>.</span><span class="sxs-lookup"><span data-stu-id="c1d65-630">In the <strong>Reason</strong> field, select <strong>Short Picking with manual reallocation</strong>.</span></span></li>
<li><span data-ttu-id="c1d65-631">Velg plasseringen/nummerskiltet i listen.</span><span class="sxs-lookup"><span data-stu-id="c1d65-631">Select the location/license plate in the list.</span></span></li>
</ol>
</td>
<td>
<ul>
<li><span data-ttu-id="c1d65-632">For gjeldende arbeid oppstår det følgende handlinger:</span><span class="sxs-lookup"><span data-stu-id="c1d65-632">On the current work, the following actions occur:</span></span>
<ul>
<li><span data-ttu-id="c1d65-633">Plukklinjen lukkes, og den plukkede kvantiteten er 0 (null).</span><span class="sxs-lookup"><span data-stu-id="c1d65-633">The pick line is closed, and the picked quantity is 0 (zero).</span></span></li>
<li><span data-ttu-id="c1d65-634">Plasseringslinjen avbrytes.</span><span class="sxs-lookup"><span data-stu-id="c1d65-634">The put line is canceled.</span></span></li>
</ul>
</li>
<li><span data-ttu-id="c1d65-635">Det opprettes en beholdningstransaksjon av <strong>Opptelling</strong>-typen, og <strong>Solgt</strong>-utstedelsestypen opprettes for å representere justeringen.</span><span class="sxs-lookup"><span data-stu-id="c1d65-635">An inventory transaction of the <strong>Counting</strong> type and the <strong>Sold</strong> issue type is created to represent the adjustment.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="c1d65-636">Alle eksisterende reservasjoner som påvirkes av kvantitetsjusteringen i plasseringen/nummerskiltet for plukking med mangler, fjernes.</span><span class="sxs-lookup"><span data-stu-id="c1d65-636">All existing reservations that are affected by the quantity adjustment in the short-picked location/license plate are removed.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="c1d65-637">Et arbeidsunntak av <strong>Plukk med mangler</strong>-typen settes opp, der <strong>Ny fordeling av vare</strong> = <strong>Automatisk</strong>, <strong>Juster beholdning</strong> = <strong>Ja/Nei</strong> og <strong>Fjern reservasjoner</strong> = <strong>Ja/Nei</strong>.</span><span class="sxs-lookup"><span data-stu-id="c1d65-637">A work exception of the <strong>Short pick</strong> type is set up, where <strong>Item reallocation</strong> = <strong>Automatic</strong>, <strong>Adjust inventory</strong> = <strong>Yes/No</strong>, and <strong>Remove reservations</strong> = <strong>Yes/No</strong>.</span></span></td>
<td><span data-ttu-id="c1d65-638">Gjelder ikke</span><span class="sxs-lookup"><span data-stu-id="c1d65-638">Not applicable</span></span></td>
<td>
<ol>
<li><span data-ttu-id="c1d65-639">Velg <strong>Plukk med mangler</strong>-menyelementet på lagerappen når du kjører plukkarbeid.</span><span class="sxs-lookup"><span data-stu-id="c1d65-639">Select the <strong>Shortpick</strong> menu item on the warehouse app when you run picking work.</span></span></li>
<li><span data-ttu-id="c1d65-640">I <strong>Kvantitet for plukking med mangler</strong>-feltet angir du <strong>0</strong> (null).</span><span class="sxs-lookup"><span data-stu-id="c1d65-640">In the <strong>Shortpick Quantity</strong> field, enter <strong>0</strong> (zero).</span></span></li>
<li><span data-ttu-id="c1d65-641">I <strong>Årsak</strong>-feltet velger du <strong>Plukking med mangler med automatisk omfordeling</strong>.</span><span class="sxs-lookup"><span data-stu-id="c1d65-641">In the <strong>Reason</strong> field, select <strong>Short Picking with automatic reallocation</strong>.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="c1d65-642">Plukking med mangler som involverer automatisk omfordeling, støttes ikke.</span><span class="sxs-lookup"><span data-stu-id="c1d65-642">Short-picking that involves automatic reallocation isn't supported.</span></span></td>
<td><span data-ttu-id="c1d65-643">Plukking med mangler som involverer automatisk omfordeling, støttes ikke.</span><span class="sxs-lookup"><span data-stu-id="c1d65-643">Short-picking that involves automatic reallocation isn't supported.</span></span></td>
</tr>
</tbody>
</table>

#### <a name="change-the-inventory-status"></a><span data-ttu-id="c1d65-644">Endre beholdningsstatusen</span><span class="sxs-lookup"><span data-stu-id="c1d65-644">Change the inventory status</span></span>

> [!NOTE]
> <span data-ttu-id="c1d65-645">Denne lagerhandlingen kan utføres fra flere inngangspunkter.</span><span class="sxs-lookup"><span data-stu-id="c1d65-645">This warehouse action can be performed from multiple entry points.</span></span> <span data-ttu-id="c1d65-646">I eksempelet som vises her, benyttes **Beholdningsstatusendring**-handlingen på **På lager etter plassering**-siden.</span><span class="sxs-lookup"><span data-stu-id="c1d65-646">The example that is shown here uses **Inventory status change** action on the **On-hand by location** page.</span></span>

<table>
<thead>
<tr>
<th><span data-ttu-id="c1d65-647">Nøkkeloppsettsparameter</span><span class="sxs-lookup"><span data-stu-id="c1d65-647">Key setup parameter</span></span></th>
<th><span data-ttu-id="c1d65-648">Partikvantitet er tilgjengelig</span><span class="sxs-lookup"><span data-stu-id="c1d65-648">Batch quantity is available</span></span></th>
<th><span data-ttu-id="c1d65-649">Nøkkelbrukertrinn</span><span class="sxs-lookup"><span data-stu-id="c1d65-649">Key user steps</span></span></th>
<th><span data-ttu-id="c1d65-650">Lagerarbeid</span><span class="sxs-lookup"><span data-stu-id="c1d65-650">Warehouse work</span></span></th>
<th><span data-ttu-id="c1d65-651">Ordreigangsatt partireservasjon</span><span class="sxs-lookup"><span data-stu-id="c1d65-651">Order-committed batch reservation</span></span></th>
</tr>
</thead>
<tbody>
<tr>
<td rowspan='2'><span data-ttu-id="c1d65-652">I <strong>Lager</strong>-fanen, i <strong>Lager</strong>-oppføringen, er <strong>Fjern reservasjoner og merkinger</strong>-feltet satt til <strong>Reservasjoner</strong> eller <strong>Merkinger og reservasjoner</strong>.</span><span class="sxs-lookup"><span data-stu-id="c1d65-652">On the <strong>Warehouse</strong> tab, in the <strong>Warehouse</strong> record, the <strong>Remove reservations and markings</strong> field is set to <strong>Reservations</strong> or <strong>Markings and reservations</strong>.</span></span></td>
<td><span data-ttu-id="c1d65-653">Ja</span><span class="sxs-lookup"><span data-stu-id="c1d65-653">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="c1d65-654">Velg en spesifikk plassering.</span><span class="sxs-lookup"><span data-stu-id="c1d65-654">Select a specific location.</span></span></li>
<li><span data-ttu-id="c1d65-655">Velg en linje som har et spesifikt element, en spesifikk plassering og et spesifikt nummerskilt (hvis plasseringen er nummerskiltstyrt).</span><span class="sxs-lookup"><span data-stu-id="c1d65-655">Select a line that has a specific item, location, and license plate (if the location is license plate–controlled).</span></span></li>
<li><span data-ttu-id="c1d65-656">Velg <strong>Beholdningsstatusendring</strong>.</span><span class="sxs-lookup"><span data-stu-id="c1d65-656">Select <strong>Inventory status change</strong>.</span></span></li>
<li><span data-ttu-id="c1d65-657">Sett <strong>Beholdningsstatus</strong>-feltet til <strong>Blokkering</strong>.</span><span class="sxs-lookup"><span data-stu-id="c1d65-657">Set the <strong>Inventory status</strong> field to <strong>Blocking</strong>.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="c1d65-658">Beholdningsstatusendringer er ikke tillatt for kvantiteter som er reservert for arbeid.</span><span class="sxs-lookup"><span data-stu-id="c1d65-658">Inventory status changes aren't allowed for quantities that are reserved for work.</span></span></td>
<td>
<ul>
<li><span data-ttu-id="c1d65-659">Kvantiteten reserveres på nytt for samme parti.</span><span class="sxs-lookup"><span data-stu-id="c1d65-659">The quantity is re-reserved for the same batch.</span></span> <span data-ttu-id="c1d65-660">Systemet tilordner tilfeldig en plassering og et nummerskilt (hvis plasseringen er nummerskiltstyrt) der kvantiteten er tilgjengelig.</span><span class="sxs-lookup"><span data-stu-id="c1d65-660">The system randomly assigns a location and license plate (if the location is license plate–controlled) where the quantity is available.</span></span></li>
<li><span data-ttu-id="c1d65-661">Det opprettes to beholdningstransaksjoner av <strong>Beholdningsstatusendring</strong>-typen for å representere endringen i beholdningsstatusdimensjonen.</span><span class="sxs-lookup"><span data-stu-id="c1d65-661">Two inventory transactions of the <strong>Inventory status change</strong> type are created to represent the change in the inventory status dimension.</span></span></li>
<li><span data-ttu-id="c1d65-662">En beholdningstransaksjon av <strong>Beholdningsblokkering</strong>-typen og <strong>Reservert fysisk</strong>-utstedelsestypen opprettes for å representere reservasjonen av den blokkerte kvantiteten.</span><span class="sxs-lookup"><span data-stu-id="c1d65-662">An inventory transaction of the <strong>Inventory blocking</strong> type and the <strong>Reserved physical</strong> issue type is created to represent the reservation of the blocked quantity.</span></span></li>
</ul>
</td>
</tr>
<tr>
<td><span data-ttu-id="c1d65-663">Nei</span><span class="sxs-lookup"><span data-stu-id="c1d65-663">No</span></span></td>
<td><span data-ttu-id="c1d65-664">Se den forrige raden.</span><span class="sxs-lookup"><span data-stu-id="c1d65-664">See the previous row.</span></span></td>
<td><span data-ttu-id="c1d65-665">Beholdningsstatusendringer er ikke tillatt for kvantiteter som er reservert for arbeid.</span><span class="sxs-lookup"><span data-stu-id="c1d65-665">Inventory status changes aren't allowed for quantities that are reserved for work.</span></span></td>
<td>
<ul>
<li><span data-ttu-id="c1d65-666">Reservasjonen fjernes.</span><span class="sxs-lookup"><span data-stu-id="c1d65-666">The reservation is removed.</span></span></li>
<li><span data-ttu-id="c1d65-667">Det opprettes to beholdningstransaksjoner av <strong>Beholdningsstatusendring</strong>-typen for å representere endringen i beholdningsstatusdimensjonen.</span><span class="sxs-lookup"><span data-stu-id="c1d65-667">Two inventory transactions of the <strong>Inventory status change</strong> type are created to represent the change in the inventory status dimension.</span></span></li>
<li><span data-ttu-id="c1d65-668">En beholdningstransaksjon av <strong>Beholdningsblokkering</strong>-typen og <strong>Reservert fysisk</strong>-utstedelsestypen opprettes for å representere reservasjonen av den blokkerte kvantiteten.</span><span class="sxs-lookup"><span data-stu-id="c1d65-668">An inventory transaction of the <strong>Inventory blocking</strong> type and the <strong>Reserved physical</strong> issue type is created to represent the reservation of the blocked quantity.</span></span></li>
</ul>
</td>
</tr>
<tr>
<td rowspan='2'><span data-ttu-id="c1d65-669">I <strong>Lager</strong>-fanen, i <strong>Lager</strong>-oppføringen, er <strong>Fjern reservasjoner og merkinger</strong>-feltet satt til <strong>Ingen</strong>.</span><span class="sxs-lookup"><span data-stu-id="c1d65-669">On the <strong>Warehouse</strong> tab, in the <strong>Warehouse</strong> record, the <strong>Remove reservations and markings</strong> field is set to <strong>None</strong>.</span></span></td>
<td><span data-ttu-id="c1d65-670">Ja</span><span class="sxs-lookup"><span data-stu-id="c1d65-670">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="c1d65-671">Velg en spesifikk plassering.</span><span class="sxs-lookup"><span data-stu-id="c1d65-671">Select a specific location.</span></span></li>
<li><span data-ttu-id="c1d65-672">Velg en linje som har et spesifikt element, en spesifikk plassering og et spesifikt nummerskilt (hvis plasseringen er nummerskiltstyrt).</span><span class="sxs-lookup"><span data-stu-id="c1d65-672">Select a line that has a specific item, location, and license plate (if the location is license plate–controlled).</span></span></li>
<li><span data-ttu-id="c1d65-673">Velg <strong>Beholdningsstatusendring</strong>.</span><span class="sxs-lookup"><span data-stu-id="c1d65-673">Select <strong>Inventory status change</strong>.</span></span></li>
<li><span data-ttu-id="c1d65-674">Sett <strong>Beholdningsstatus</strong>-feltet til <strong>Blokkering</strong>.</span><span class="sxs-lookup"><span data-stu-id="c1d65-674">Set the <strong>Inventory status</strong> field to <strong>Blocking</strong>.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="c1d65-675">Beholdningsstatusendringer er ikke tillatt for kvantiteter som er reservert for arbeid.</span><span class="sxs-lookup"><span data-stu-id="c1d65-675">Inventory status changes aren't allowed for quantities that are reserved for work.</span></span></td>
<td><span data-ttu-id="c1d65-676">Kvantiteten reserveres på nytt for samme parti.</span><span class="sxs-lookup"><span data-stu-id="c1d65-676">The quantity is re-reserved for the same batch.</span></span> <span data-ttu-id="c1d65-677">Systemet tilordner tilfeldig en plassering og et nummerskilt (hvis plasseringen er nummerskiltstyrt) der kvantiteten er tilgjengelig.</span><span class="sxs-lookup"><span data-stu-id="c1d65-677">The system randomly assigns a location and license plate (if the location is license plate–controlled) where the quantity is available.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="c1d65-678">Nei</span><span class="sxs-lookup"><span data-stu-id="c1d65-678">No</span></span></td>
<td><span data-ttu-id="c1d65-679">Se den forrige raden.</span><span class="sxs-lookup"><span data-stu-id="c1d65-679">See the previous row.</span></span></td>
<td><span data-ttu-id="c1d65-680">Beholdningsstatusendringer er ikke tillatt for kvantiteter som er reservert for arbeid.</span><span class="sxs-lookup"><span data-stu-id="c1d65-680">Inventory status changes aren't allowed for quantities that are reserved for work.</span></span></td>
<td><span data-ttu-id="c1d65-681">Beholdningsstatusendringer er ikke tillatt.</span><span class="sxs-lookup"><span data-stu-id="c1d65-681">Inventory status changes aren't allowed.</span></span></td>
</tr>
</tbody>
</table>

## <a name="limitations"></a><span data-ttu-id="c1d65-682">Begrensninger</span><span class="sxs-lookup"><span data-stu-id="c1d65-682">Limitations</span></span>

- <span data-ttu-id="c1d65-683">Den fleksible funksjonaliteten for dimensjonsreservasjon på lagernivå støtter ikke følgende funksjoner:</span><span class="sxs-lookup"><span data-stu-id="c1d65-683">The flexible warehouse-level dimension reservation functionality doesn't support the following features:</span></span>

    - <span data-ttu-id="c1d65-684">Administrering av faktisk vekt</span><span class="sxs-lookup"><span data-stu-id="c1d65-684">Catch weight management</span></span>
    - <span data-ttu-id="c1d65-685">Fysisk negativt lager</span><span class="sxs-lookup"><span data-stu-id="c1d65-685">Physical negative inventory</span></span>
    - <span data-ttu-id="c1d65-686">Reservasjon mot bestilt forsyning</span><span class="sxs-lookup"><span data-stu-id="c1d65-686">Reservation against ordered supply</span></span>
    - <span data-ttu-id="c1d65-687">Plukking av overføringsordrer og råvarer</span><span class="sxs-lookup"><span data-stu-id="c1d65-687">Transfer orders and raw material picking</span></span>

- <span data-ttu-id="c1d65-688">Regelen for konsolidering av containere for pakking etter direktivenhet har begrensninger.</span><span class="sxs-lookup"><span data-stu-id="c1d65-688">The container consolidation rule for packing by directive unit has limitations.</span></span> <span data-ttu-id="c1d65-689">For ordreigangsatte reservasjoner anbefaler vi at du ikke bruker maler for containerbygging i tilfeller der **Pakk etter direktivenhet**-feltet er aktivert.</span><span class="sxs-lookup"><span data-stu-id="c1d65-689">For order-committed reservations, we recommend that you not use container build templates where the **Pack by directive unit** field is enabled.</span></span> <span data-ttu-id="c1d65-690">I gjeldende utforming brukes ikke plasseringsdirektiver når det opprettes lagerarbeid.</span><span class="sxs-lookup"><span data-stu-id="c1d65-690">In the current design, location directives aren't used when warehouse work is created.</span></span> <span data-ttu-id="c1d65-691">Derfor brukes bare den laveste enheten i enhetssekvensgruppen (beholdningsenheten) under bølgetrinnet containerbruk.</span><span class="sxs-lookup"><span data-stu-id="c1d65-691">Therefore, only the lowest unit in the unit sequence group (the inventory unit) is applied during the containerization wave step.</span></span>
