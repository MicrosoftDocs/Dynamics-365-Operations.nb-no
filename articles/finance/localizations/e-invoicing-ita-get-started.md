...
3. På siden for **oppsett av funksjonsversjon** velger du kategorien **Handlinger** for å behandle listen over handlinger. Handlinger definerer en liste over operasjoner som må kjøres i sekvensiell rekkefølge for å oppnå full utførelse av hendelsen.
    ...
    |-----------|--------------------|--------------------------------------------------------|
    | 1         | Transformer dokument | Opprett XML-filen for e-faktura i **FatturaPA**-format. |
...
> [!NOTE]
> Før du fortsetter må du kontrollere at oppsettet som kreves for italienske e-fakturaer, ble fullført. Hvis du vil ha mer informasjon, kan du se [Elektroniske fakturaer for kunde](./emea-ita-e-invoices.md). Vær oppmerksom på at noen av oppsettrinnene som beskrives i dette emnet, kanskje ikke er tilgjengelige på grunn av aktivering av Elektronisk fakturering.
