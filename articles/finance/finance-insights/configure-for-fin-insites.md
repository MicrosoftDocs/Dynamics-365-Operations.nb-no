...
Finance Insights kombinerer funksjonalitet fra Microsoft Dynamics 365 Finance med Common Data Service, Azure og AI Builder for å gi organisasjonen kraftige prognoseverktøy. Dette emnet forklarer konfigurasjonstrinnene som gjør at systemet kan bruke funksjonene i Finance Insights.
...
3. Hvis du bruker Contoso-demonstrasjonsdata, må du ha flere eksempeldata for å kunne bruke funksjonene for kundebetalingsprognoser, kontantstrømprognoser og budsjettprognoser. 

## <a name="configure-common-data-service"></a>Konfigurer Common Data Service

Du kan fullføre de manuelle konfigurasjonstrinnene nedenfor, eller du kan gjøre konfigurasjonsprosessen raskere ved å bruke Windows PowerShell-skriptet du også finner nedenfor. Når PowerShell-skriptet er ferdig å kjøre, vil det gi deg verdier du kan bruke til å konfigurere Finance Insights. 

> [!NOTE]
> Åpne PowerShell på PC-en for å kjøre skriptet. Du må kanskje ha PowerShell versjon 5. Microsoft Azure CLI-alternativet «Prøv det» fungerer kanskje ikke.

# <a name="manual-configuration-steps"></a>[Manuelle konfigurasjonstrinn](#tab/configuration-steps)

1. Åpne [administrasjonssenteret for Power Platform](https://admin.powerplatform.microsoft.com/), og følg denne fremgangsmåten for å opprette et nytt Common Data Service-miljø i den samme Active Directory-leieren:
    ...
    2. Velg **Nytt miljø**.
    3. I **Type**-feltet velger du **Sandkasse**.
    4. Sett alternativet **Opprett database** til **Ja**.
    5. Velg **Neste**.
    6. Velg språket og valutaen for organisasjonen.
    7. Godta standardverdiene for de andre feltene.
    8. Velg **Lagre**.
    9. Oppdater **Miljøer**-siden.
    10. Vent til verdien i **Tilstand**-feltet oppdateres til **Klar**.
    11. Noter ID-en for Common Data Service-organisasjonen.
    12. Velg miljøet, og velg deretter **Innstillinger**.
    ...
    16. Sett feltet **ID for referanseinformasjon for forekomst** til verdien for ID-en for Common Data Service-organisasjon du noterte tidligere.
    17. Noter deg URL-adressen for Common Data Service-organisasjonen på adresselinjen i nettleseren. URL-adressen kan for eksempel være `https://org42b2b3d3.crm.dynamics.com`.
...
# <a name="windows-powershell-configuration-script"></a>[Konfigurasjonsskript for Windows PowerShell](#tab/powershell-configuration-script)
...
### <a name="enter-the-common-data-service-directory-id-and-the-users-azure-ad-object-id"></a>Angi katalog-ID-en for Common Data Service og brukerens objekt-ID for Azure AD

1. Angi katalog-ID-en for Common Data Service:

    1. Åpne [Azure-portalen](https://portal.azure.com).
    2. Logg på ved å bruke bruker-ID-en som ble brukt til å opprette Common Data Service-miljøet.
...
2. Velg **PowerShell**.
3. Opprett lagringsplass hvis du blir bedt om det. Deretter laster du opp Windows PowerShell-skriptet til økten.
4. Kjør skriptet.
5. Følg instruksjonene for å kjøre skriptet.
6. Bruk informasjonen fra skriptutdataene til å installere tillegget **Eksporter til Data Lake** i LCS.
7. Bruk informasjonen fra skriptutdataene til å aktivere enhetslageret på **Datatilkoblinger**-siden i Finance (**Systemadministrasjon \> Systemparametere \> Datatilkoblinger**).
...
> [!NOTE]
> Kontroller at du oppretter følgende ressurser i samme Azure AD-forekomst som Common Data Service-miljøet. Du kan ikke bruke ressurser fra en annen Azure AD-forekomst.
...
4. Angi følgende verdier på siden med detaljer for tillegget **Få innsikt**.
    ...
    | Verdi                                                    | Beskrivelse |
    |----------------------------------------------------------|-------------|
    | URL-adresse for CDS-organisasjon                                     | URL-adressen for Common Data Service-organisasjonen for Common Data Service-forekomsten. Du finner denne verdien ved å åpne [Power Apps-portalen](https://make.powerapps.com), velge **Innstillinger**-knappen (tannhjulsymbolet) i øvre høyre hjørne, velge **Avanserte innstillinger** og kopiere URL-adressen. (URL-adressen slutter på «dynamics.com».) |
    | ID for CDS-organisasjon                                               | Miljø-ID-en for Common Data Service-forekomsten. Du finner denne verdien ved å åpne [Power Apps-portalen](https://make.powerapps.com), velge **Innstillinger**-knappen (tannhjulsymbolet) i øvre høyre hjørne, velge **Tilpassinger \> Utviklerressurser \> ID for referanseinformasjon for forekomst** og kopiere **ID**-verdien. |
    | ID for CDS-leier (katalog-ID fra AAD)               | Leier-ID-en for Common Data Service-forekomsten. Du finner denne verdien ved å åpne [Azure-portalen](https://portal.azure.com), gå til **Azure Active Directory** og kopiere verdien for **Leier-ID**. |
    | Angi brukerobjekt-ID for personen som har rollen Systemansvarlig | Azure AD-brukerobjekt-ID-en for brukeren i Common Data Service. Denne brukeren må være en systemansvarlig for Common Data Service-forekomsten. Du finner denne verdien ved å åpne [Azure-portalen](https://portal.azure.com), gå til **Azure Active Directory\> Brukere**, velger brukeren og deretter kopiere verdien for **Objekt-ID** i **Identitet**-delen. |
    | Er dette standard CDS-miljø for leieren?      | Hvis Common Data Service-forekomsten var den første produksjonsforekomsten som ble opprettet, merker du av her. Hvis Common Data Service-forekomsten ble opprettet manuelt, fjerner du merket her. |
