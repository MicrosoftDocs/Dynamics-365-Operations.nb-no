...
I Microsoft Dynamics 365 Retail kan forhandlere bruke klientbokfunksjonaliteten til å hjelpe butikkmedarbeidere med å skape langsiktige forbindelser med viktige kunder.

Klientboken inneholder kundekort som viser kontaktinformasjon for hver kunde, sammen med tre ekstra egenskaper som er definert av forhandleren og konfigurert i Retail Headquarters. Forhandlere kan bestemme de tre viktigste tingene som salgsmedarbeidere bør vite om kunder. En smykkeforhandler kan for eksempel inkludere viktige datoer som jubileer eller fødselsdager, fordi disse datoene er anledninger da folk kjøper flere smykker. På samme måte kan en moteforhandler ta med kundens foretrukne handleinteresser og merker.
...
For å hente den kritiske informasjonen som kunder deler, kan salgsmedarbeidere ikke bare bruke klientbokattributtene, men også bruke funksjonaliteten aktiviteter og merknader. Forhandlere kan konfigurere aktivitetstypene, for eksempel informasjon om butikkbesøk, e-postadresse, telefonnummer og avtaler. Aktiviteter som medarbeidere oppretter, kan vises i et tids linjeformat på salgsstedet. De kan også vises i kategorien **Aktiviteter** på siden **Alle kunder \> Generelt** i Retail Headquarters.

Salgsmedarbeidere kan du også bruke merknader til å registrere generell kundeinformasjon som det kan refereres til før alle typer samhandlinger. Disse merknadene lagres i Retail Headquarters og kan vises i kundeprofilen eller på siden for kundedetaljer i telefonsenteret.
...
1. I arbeidsområdet **funksjonsbehandling** filtrerer du funksjonene etter modulen **Detaljhandel og handel**.
    ...
    ![Kundeaktiviteter i listen over funksjoner i modulen Detaljhandel og handel](./media/Enable_clienteling.png "Kundeaktiviteter i listen over funksjoner i modulen Detaljhandel og handel")

2. Aktiver **Kundeaktiviteter**-funksjonen ved å velge **Aktiver nå**.
3. På siden **Detaljhandelsparametere**, i kategorien **Nummerserie**, velger du raden **Klientbokidentifikator**. Deretter velger du en nummerserie i feltet **Nummerseriekode**. Systemet vil bruke denne nummerserien til å tilordne en ID til klientbøker.
4. Velg **Lagre**.
...
6. Velg attributtgruppen du nettopp opprettet, på siden **Detaljhandelsparametere** i kategorien **Kundeaktiviteter** i feltet **Attributtgruppe for klientbok**.

    ![Attributtgruppe for klientbok valgt](./media/Client%20book%20attributes.png "Attributtgruppe for klientbok valgt")

7. Hvis du vil registrere aktiviteter som forekommer på salgsstedet, definerer du aktivitetstypene på **Aktivitetstyper**-siden (**Retail \> Kunder \> Aktivitetstyper**).

    > [!NOTE]
    > Aktivitetstyper hentes av detaljhandelsserveren når den foretar et sanntidskall for første gang. Når aktivitetene hentes, hurtigbufres de i et par timer. Hvis du endrer aktivitetstypene, må du vente til hurtigbufferen ikke lenger er gyldig. Du kan også starte Retail Server-tjenesten for ikke-produktive miljøer på nytt.
...
## <a name="turn-on-the-integration-of-customer-insights-with-retail"></a>Slå på integrering av Customer Insights med Retail

Hvis du vil slå på integreringen av Customer Insights med Retail, må du kontrollere at du har en aktiv forekomst av Customer Insights i leieren der Retail er klargjort. Du må også ha en Azure Active Directory (Azure AD)-brukerkonto som har et Azure-abonnement.
...
7. Følg denne fremgangsmåten i Retail på siden **Retail-parametere** i kategorien **Kundeaktiviteter** i **Dynamics 365 Customer Insights**-hurtigfanen:
    ...
    5. I feltet **Alternativ kunde-ID** angir du egenskapen i Customer Insights som er tilordnet kundekontonummeret. (I Retail er kundens kontonummer kunde-IDen.)
