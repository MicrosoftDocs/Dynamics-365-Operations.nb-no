...
ms.locfileid: "1565817"
---
# <a name="distributed-order-management-dom"></a>Behandling av distribuert ordre (DOM)

[!include [banner](includes/banner.md)]

I dette nye paradigmet for detaljhandelsoperasjoner forsøker forhandlere å formidle tilpasset kundeengasjement, omnikanalopplevelser og problemfrie samhandlinger. Veldig mange alternativer er tilgjengelige, og derfor vil brukerne handle der de får den beste opplevelsen. I mange tilfeller er priser og produkter ikke lenger de mest avgjørende faktorene for forbrukere.
...
2. Vis **Retail**-noden i kategorien **Konfigurasjonsnøkler**, og merk deretter av for **Behandling av distribuert ordre**.
3. Gå til **Retail \> Behandling av distribuert ordre \> Oppsett \> DOM-parametere**.
4. Angi følgende verdier i kategorien **Generelt**:
    ...
    - **Bekreft bruk av Bing-kart for DOM** – Sett dette alternativet til **Ja**.
        ...
        > [!NOTE]
        > Du kan sette dette alternativet til **Ja** bare hvis alternativet **Aktiver Bing-kart** i kategorien **Bing-kart** på siden **Delte parametere for detaljhandel** (**Retail \> Hovedkvarteroppsett \> Parametere \> Delte parametere for detaljhandel**) også er satt til **Ja**, og hvis en gyldig nøkkel er angitt i feltet **Nøkkel for Bing-kart**.
...
6. Gå tilbake til **Retail \> Behandling av distribuert ordre \> Oppsett \> DOM-parametere**.
...
8. DOM-funksjonen støtter definisjonen av ulike typer DOM-regler, og organisasjoner kan konfigurere flere regler, avhengig av deres forretningsbehov. DOM-regler kan defineres for en gruppe med lokasjoner eller individuelle lokasjoner, og for en bestemt produktkategori, produkt eller variant. Hvis du vil opprette gruppering av lokasjoner som må brukes for DOM-reglene, gjør du følgende:
    ...
    1. Gå til **Retail \> Kanaloppsett \> Oppfyllelsesgrupper**.
...
9. Hvis du vil definere regler, går du til **Retail \> Behandling av distribuert ordre \> Oppsett \> Administrer regler**. Følgende DOM-regler støttes for øyeblikket:
...
10. Oppfyllelsesprofiler brukes til å gruppere en samling med regler, juridiske enheter, salgsordreopprinnelser og leveringsmåter. Alle DOM-kjøringer gjelder for en bestemt oppfyllelsesprofil. På denne måten kan organisasjoner definere og kjøre et sett med regler for et sett med juridiske enheter for ordrer som har bestemte salgsordreopprinnelser og leveringsmåter. Hvis ulike sett med regler må kjøres for ulike typer salgsordreopprinnelser eller leveringsmåter, kan oppfyllelsesprofilene derfor defineres i henhold til dette. Gjør følgende for å konfigurere oppfyllelsesprofiler:  
    ...
    1. Gå til **Retail \> Behandling av distribuert ordre \> Oppsett \> Oppfyllelsesprofiler**.
    2. Velg **Ny**.
...
1. Gå til **Retail \> Behandling av distribuert ordre \> Satsvis behandling \> Oppsett for DOM-prosessorjobb**.
...
- Ordrelinjer som oppfyller kriteriene for salgsordreopprinnelse, leveringsmåter og juridisk enhet, slik det er definert i DOM-profilen, og som også oppfyller noen av disse kriteriene:
    ...
    - De er opprettet fra detaljhandelskanaler.
...
1. Gå til **Retail \> Behandling av distribuert ordre \> Behandling av distribuert ordre**.
...
Følgende beskriver innstillinger på ordrelinjen. Hvis du vil åpne ordrelinjen, går du til **Retail \> Kunder \> All salgsordrer**.
...
1. Gå til **Retail \> Behandling av distribuert ordre \> Satsvis behandling \> Oppsett for jobb for sletting av DOM-oppfyllelsesdata**. 
