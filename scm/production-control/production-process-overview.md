---
title: Oversikt over prosessen for produksjon
description: Denne artikkelen gir en oversikt over produksjonsprosessen. Den beskriver de ulike stadiene av produksjonsordrer, partiordrer og Kanbaner, fra oppretting av til lukking av regnskapsperioden.
author: YuyuScheller
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
# <a name="production-process-overview"></a>Oversikt over prosessen for produksjon

Denne artikkelen gir en oversikt over produksjonsprosessen. Den beskriver de ulike stadiene av produksjonsordrer, partiordrer og Kanbaner, fra oppretting av til lukking av regnskapsperioden. 
...
Den **produksjonskontroll** modulen som er knyttet til andre moduler, **behandling av produktinformasjon**, **Inventory management**, **Økonomi**, **Lokasjonsstyring**, **prosjektregnskap**, og **organisasjonsadministrasjon**. Denne integreringen støtter informasjonsflyten som er nødvendig for å fullføre produksjonen av en ferdig vare.  

Produksjonsprosessen påvirkes vanligvis av kostnadsregnskaps- og beholdningsvurderingsmetodene som er valgt for en bestemt produksjonsprosess. Dynamics 365 for operasjoner støtter både faktiske kostnader (først inn, først ut \[FIFO\]; sist inn, først ut \[LIFO\]; glidende gjennomsnitt; og periodiske avveid gjennomsnitt) og metoder for standardkost. Lean manufacturing implementeres basert på backflush-etterkalkuleringsprinsippet.  
...
Ulike produkter og produksjonstopologier krever bruk av forskjellige ordretyper. Dynamics 365 for operasjoner kan bruke ulike ordretyper i blandet modus. Med andre ord kan alle ordretyper forekomme under hele prosessen med å produsere ett ferdig produkt.
...
2.  **Estimert** – Du kan beregne estimater for material- og ressursforbruk. Estimeringen genererer lagertransaksjoner for råmaterialer med statusen **I ordre**. Mottak for primære produkter, koprodukter og biprodukter er genererer når produksjonsordrer eller satsvis ordrer blir estimert. Hvis Stykklisten inneholder linjer med den **tilknyttet forsyning** skriver du inn bestillinger for materialer eller tjenester for underleveranse operasjonen er generert og tilknyttet produksjonsordren eller parti. Varer eller ordrer reserveres i henhold til reserveringsstrategien i produksjonsordren, og prisen på de ferdige varene beregnes basert på parameterinnstillingene.
...
5.  **Klargjort**/**plukket** – når alle materialer og ressurser har blitt klargjort på produksjon lokasjonen, produksjonsstykklistelinjene eller kanban-linjer oppdateres til statusen **plukket**. Tilknyttede forsyningsordrer og tilknyttet lagerarbeid fullføres vanligvis på dette stadiet. Kanban-kortene eller jobbkortene som er nødvendige for å rapportere fremdrift for produksjonen, må tilordnes og skrives ut.
