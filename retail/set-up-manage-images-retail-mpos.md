---
title: Definer og administrer bilder for moderne Priskontroll
description: Denne artikkelen forklarer trinnene som er involvert i oppretting og administrasjon av avbildninger for de forskjellige enhetene som vises i Retail moderne POS (MPOS).
author: MargoC
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
# <a name="set-up-and-manage-images-for-retail-modern-pos"></a>Definer og administrer bilder for moderne Priskontroll

Denne artikkelen forklarer trinnene som er involvert i oppretting og administrasjon av avbildninger for de forskjellige enhetene som vises i Retail moderne POS (MPOS).
...
Bildene som vises i Retail moderne POS (MPOS) må være lagret eksternt, utenfor Microsoft Dynamics 365 for operasjoner - handel. De er vanligvis vertsbasert i et innholdsbehandlingssystem, et nettverk for innholdslevering (CDN) eller på en mediaserver. MPOS henter og viser deretter bildene for de aktuelle enhetene, for eksempel produkter og kataloger, ved å åpne mål-URL-adressen. For å hente disse eksternt vertsbaserte bildene krever MPOS riktig URL-format for bildene. Du kan konfigurere det nødvendige URL-formatet for bildene ved å sette opp verdien for **Primær URL-adresse for media** i Kanalprofilen og bruke funksjonaliteten **Definer mal for medier** for hver enhet. Du kan også overskrive standard URL-formatet for et delsett av enheter ved hjelp av funksjonaliteten **Rediger i Excel**. **Viktig merknad:** i den gjeldende versjonen av Dynamics 365 for operasjoner, du kan ikke definere URL-formatet ved hjelp av den **bilde** attributtet XML for MPOS i den **standard** attributtgruppe for enheter. Hvis du er fortrolig med Microsoft Dynamics AX 2012 R3 og nå bruker den gjeldende versjonen av Dynamics 365 for operasjoner, må du kontrollere at du alltid bruke den nye **Definer media mal** funksjonalitet til å sette opp bilder. Ikke bruk eller endre **Bilde**-attributtet i **Standard**-attributtgruppen for noen enheter, inkludert produkter. Endringer du gjør direkte i **Standard**-attributtgruppen for bilder gjenspeiles ikke. Dette alternativet deaktiveres i fremtidige versjoner. I prosedyrene nedenfor er bilder definert for enheten Katalog som et eksempel. Disse prosedyrene vil hjelpe med å garantere at den riktige bildemålbanen er implisitt definert for alle katalogbilder som bruker en felles bane. For eksempel hvis du har satt opp en media-server eller CDN eksternt, og vil at bildene skal vises i MPOS for en gitt butikk, hjelper **Definer mal for medier**-funksjonen deg med å sette banen der MPOS kan søke etter og hente bildene. **Obs!** For dette demodataeksemplet er medieserveren distribuert på detaljhandelsserveren. Du kan imidlertid ha det hvor som helst utenfor Dynamics 365 for operasjoner.

### <a name="set-up-the-media-base-url-for-a-channel"></a>Sette opp primær URL-adresse for media for en kanal

1.  Åpne Dynamics-365 for operasjoner HQ portal.
2.  Klikk **Retail og commerce**&gt;**kanaloppsett**&gt;**kanal profiler**. [![kanal-profil1](./media/channel-profile1.png)](./media/channel-profile1.png)
3.  I Kanalprofilen som butikken bruker for MPOS, kan du oppdatere **Primær URL-adresse for media** feltet med den primære URL-adressen for media-serveren eller CDN. I URL-adressen er den første delen av URL-adressen som er felles for alle bildemapper av ulike enheter. [![kanal-profile2](./media/channel-profile2.png)](./media/channel-profile2.png)

### <a name="define-the-media-template-for-an-entity"></a>Definere mediemalen for en enhet

1.  Klikk **Retail og commerce**&gt;**Catalog management**&gt;**katalog bilder**.
...
9.  Hvis du vil oppdatere media malen for katalogfletting bilder på kanal-side, må du kjøre **katalog jobb 1150** fra **Retail IT**&gt;**distribusjonsplan**. [![catalog1](./media/catalog1.png)](./media/catalog1.png)

## <a name="previewing-an-image-from-the-entity-level"></a>Forhåndsvisning av et bilde fra enhetsnivå
1.  Du kan forhåndsvise bildet som bruker URL-adressen for bildet som er avledet fra mediemalen fra siden for enhetsvaren i HK. For eksempel kan du gå til den aktuelle katalogen, og klikk på handlingsruten **Media**&gt;**bilder**. Bruk fra rullegardinlisten for å velge forskjellige butikker som kan ha forskjellige kanalprofiler.
...
4.  Når du er ferdig med å forhåndsvise et bilde og gjøre eventuelle endringer du krever, starter forekomsten av MPOS for den aktuelle butikken, og se om katalog-bilder vises. [![catalog4](./media/catalog4.png)](./media/catalog4.png)
...
For alle de fem støttede enhetene (arbeider, kunde, katalog, kategori og produkter), hver gang du oppdaterer den **Definer media mal** dialogboks for å sette opp et bilde, må du kontrollere at du kjører jobben Catalog (1150) fra **Retail IT**&gt;**distribusjonsplan**. Denne jobben bil aktivere den oppdaterte mediamalen for synkronisering til kanalen, og bruk av MPOS. Kjør Catalog job (1150) etter at du gjør hvilken som helst av følgende endringer:

-   Du oppdaterer katalog bilde media mal fra **katalog bilder**&gt;**Definer media mal**.
-   Du oppdaterer ansatt bilde media mal fra **ansatte bilder**&gt;**Definer media mal**.
-   Du kan oppdatere bildet media kundemalen fra **kunden bilde**&gt;**Definer media mal**.
-   Du har oppdatert produktet bilde media mal fra **produktbilder**&gt;**Definer media mal**.
-   Du oppdaterer kategorien bilde media mal fra **kategori bilder**&gt;**Definer media mal**. Du må også publisere kanalen.
...
4.  På den **Generer bilde-URL-adresser for Excel** hurtigkategorien, klikker du **Generer**. **Viktig:** når mediemalen endres, må du klikke **Generer** før du kan bruke funksjonalitet for Rediger i Excel. [![excel1](./media/excel1.jpg)](./media/excel1.jpg) du nå se en forhåndsvisning av bildet URL-adressene som ble generert basert på den siste lagrede media-malen. [![excel2](./media/excel2.png)](./media/excel2.png)**notat:** URL-adressen som er generert for Excel bruke banen og konvensjoner for media-malen som er definert. Disse konvensjonene inkluderer konvensjoner for filnavn. Forventningen er at du har angitt de fysiske bildene utenfor Dynamics AX, og at bildene kan hentes fra URL-adressene som er avledet fra mediemalen som du tidligere har definert. Du kan overskrive disse avledede URL-adresser ved hjelp av kommandoen Rediger i Excel-funksjonalitet.
5.  Klikk **Rediger i Excel**.
6.  Når Microsoft Excel-regnearket åpnes, klikker du **Aktiver redigering** når du blir bedt om det.
7.  Når du blir bedt om det, klikker du **Klarer tillegget** i høyre rute, og vent til tillegget har fullført installasjonen. [![Stole på tillegget](./media/excel4.jpg)](./media/excel4.jpg)
8.  Hvis du blir bedt om å logge på, Skriv inn påloggingsinformasjonen du brukte til å logge på HK. [![Logg på ledeteksten](./media/excel5.png)](./media/excel5.png)
...
11. Du kan overskrive bilde URL-adressene for alle enheter unntatt produkter. Endre eksisterende bilde-URL-EN, slik at den bruker den nye Måladressen for bildet, og oppdater filnavnet med det nye filnavnet for bildefilen. Filnavnet må være unikt for å garantere at posten er unik. [![Overskrive bildet URL-adresser i Excel](./media/excel6.jpg)](./media/excel6.jpg)**notat:** når du overskriver bildet URL-adresser for produkter-enheter ved hjelp av kommandoen Rediger i Excel-funksjonalitet eller siden enhet element, MPOS alltid viser alle mediene mal bildet URL-adresser med overskrives bildet URL-adresser.
...
1.  Klikk **kataloger**&gt;**Media**&gt;**bilder**, og velg katalogen bildet for å oppdatere.
...
4.  Klikk **OK**. Bilde-URL-en blir oppdatert for dette katalogbildet, og det vises en forhåndsvisning. [![preview3](./media/preview3.png)](./media/preview3.png)
5.  Du kan også se forhåndsvisningen av bildet for alle overskrevne bilde-URL-adresser på **Katalogbilder** gallerisiden.

**[![Forhåndsvisning-4](./media/preview-4.png)](./media/preview-4.png)notat:** for øyeblikket galleriet viser ikke forhåndsvisning for media mal bildet URL-adresser. For enhetene katalog, arbeider, kunde og kategori, hvis brukeren eksplisitt angir en URL-adresse fra denne siden, anbefaler vi at du angir hvilket bilde er standardbilde, fordi Detaljhandelsserver-klienter viser bare ett bilde per katalog, kunde, arbeider og kategori. Hvis brukeren ikke angir et standardbilde, bestemmer systemet standardbildet og sender det til Detaljhandelstjeneste anroper (MPOS eller e-handel).
...
**[![cat3](./media/cat3.png)](./media/cat3.png)notat:** etter kategori bilde-tilknytning, må du publisere kanalen og kjøre jobben kanal for å garantere at endringene publiseres til kanalen databasen.
...
1.  Klikk **produktinformasjonsbehandling**&gt;**produkter**&gt;**produkter**.
...
7.  Plasser MPOS i frakoblet modus. Du bør se bildet som du lastet opp for det aktuelle produktet i HK. [![offline1](./media/offline1.png)](./media/offline1.png)
...
1.  Gå til katalogen, og klikk på handlingsruten **Media**&gt;**bilder**.
